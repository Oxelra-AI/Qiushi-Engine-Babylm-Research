#!/usr/bin/env python3
"""Prefill research bidirectional full-eval payloads from no-AoA trajectory records.

This copies BLiMP/Supplement/EWoK/Entity/COMPS/GlobalPIQA/Reading outputs that were
already produced by the research no-AoA trajectory screen, so the full-eval runner only
needs to add SuperGLUE and AoA.  Candidate targets should be frozen from the no-AoA
ranker before invoking this script.  No AoA/CDI information is read here.
"""
...
TRAJ_ROOT = _public_path('experiments/archive/compact_experience/data/bidir_trajectory_screen')
...
for seed in ["43022", "43122"]:
    for m in [10, 20, 30, 40, 50, 60, 70, 75, 80, 85, 90, 95, 100]:
        TARGET_TO_TRAJ[f"bidir_seed{seed}_{m}M"] = (f"bidir_seed{seed}", f"chck_{m}M")
...
def per_checkpoint_record(traj_target: str, ckpt: str, col: str) -> Dict[str, Any]:
    p = _public_path('experiments/archive/compact_experience/data/bidir_trajectory_screen/per_checkpoint') / traj_target / ckpt / f"{col}.json"
    if not p.exists():
...
def prefill(target: str, gpu: int) -> Dict[str, Any]:
    ...
    if target not in TARGET_TO_TRAJ:
        raise KeyError(f"target is not a bidirectional trajectory-prefill target: {target}")
    ...
    payload: Dict[str, Any] = {
        "target": target,
        "description": str(target_cfg.get("description", "")) + " Prefilled from research no-AoA bidirectional trajectory records; SuperGLUE/AoA run separately.",
