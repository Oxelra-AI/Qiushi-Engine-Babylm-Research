#!/usr/bin/env python3
"""Prefill official-geometry full-eval payloads from no-AoA trajectory records."""
...
import full_eval_official_geometry_candidates as cand  # noqa: E402
import full_overall_eval_runner as base0  # noqa: E402

WORKSPACE = _public_path('experiments/archive/compact_experience')
TRAJ_ROOT = _public_path('experiments/archive/compact_experience/data/official_geometry_trajectory_screen')
OUT_ROOT = _public_path('experiments/archive/compact_experience/data/official_geometry_full_eval_candidates')
...
for family in ["official160_b256", "official_cap120geom_b256", "official_cap120geom_b286_lr2515", "official_cap120geom_b288_lr2515", "official_cap120geom_b295_lr2442", "official160_b223_lr2809"]:
...
def per_checkpoint_record(traj_target: str, ckpt: str, col: str) -> Dict[str, Any]:
    p = _public_path('experiments/archive/compact_experience/data/official_geometry_trajectory_screen/per_checkpoint') / traj_target / ckpt / f"{col}.json"
    if not p.exists():
...
def prefill(target: str, gpu: int) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        "target": target,
        "description": str(target_cfg.get("description", "")) + " Prefilled from research official-geometry no-AoA trajectory records; SuperGLUE/AoA run separately.",
