#!/usr/bin/env python3
"""Score endpoint-aware research mask full-eval payloads with corrected AoA units."""
...
OUT = _public_path('experiments/archive/compact_experience/data/mask_endpoint_full_eval/mask_endpoint_full_eval_summary.json')
NOTE = _public_path('research/notes/compact_experience/mask_endpoint_full_eval_summary.md')
...
def main() -> None:
    ...
    payload = {"status": "MASK_ENDPOINT_FULL_EVAL_SUMMARY", "baseline_clean_qwen_100M": BASELINE, "visible_leader": VISIBLE_LEADER, "candidates": rows}
    _public_path('experiments/archive/compact_experience/data/mask_endpoint_full_eval').mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# research endpoint-aware mask full evaluation", "", f"Clean-Qwen complete frontier: {BASELINE:.6f}; visible leader: {VISIBLE_LEADER:.3f}.", "",
