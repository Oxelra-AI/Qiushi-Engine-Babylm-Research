#!/usr/bin/env python3
"""Score endpoint-aware research tail full-eval payloads with corrected AoA units."""
...
PER = _public_path('experiments/archive/compact_experience/data/tail_endpoint_full_eval/per_target')
OUT = _public_path('experiments/archive/compact_experience/data/tail_endpoint_full_eval/tail_endpoint_full_eval_summary.json')
NOTE = _public_path('research/notes/compact_experience/tail_endpoint_full_eval_summary.md')
...
def score_target(path: pathlib.Path) -> dict:
    ...
    if endpoint_frozen and isinstance(oo, dict):
        ...
        payload["endpoint_frozen_submission_caveat"] = "For endpoints earlier than 100M, later strict-small AoA names are plateaued at endpoint weights. This avoids future exposure but is endpoint-frozen official-code-compatible measurement, not an ordinary submit-ready full ladder unless official server accepts it."
    ...
    return {
        ...
        "endpoint_ladder_manifest": payload.get("endpoint_ladder_manifest"),
        "endpoint_lineage_policy": payload.get("endpoint_lineage_policy"),
...
def main() -> None:
    ...
    payload = {
        "status": "TAIL_ENDPOINT_FULL_EVAL_SUMMARY",
        "baseline_clean_qwen_100M": BASELINE,
        "visible_leader": VISIBLE_LEADER,
        "candidates": rows,
        "note": "Each target is an endpoint-consistent evaluation root. For endpoints earlier than 100M, later standard AoA names are plateaued at endpoint weights; no later-trained checkpoint enters the acquisition trajectory. Such <100M results are endpoint-frozen official-code-compatible measurements, not ordinary submit-ready full-ladder results unless accepted by the official server.",
    }
    _public_path('experiments/archive/compact_experience/data/tail_endpoint_full_eval').mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# research endpoint-aware tail restart full evaluation",
        "",
        f"Clean-Qwen complete frontier: {BASELINE:.6f}; visible leader: {VISIBLE_LEADER:.3f}.",
        "",
