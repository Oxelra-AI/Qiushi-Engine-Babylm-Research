#!/usr/bin/env python3
"""Score research tail-restart full evaluation with corrected AoA units."""
...
PER = _public_path('experiments/archive/compact_experience/data/tail_restart_full_eval/per_target')
OUT = _public_path('experiments/archive/compact_experience/data/tail_restart_full_eval/tail_restart_full_eval_summary.json')
NOTE = _public_path('research/notes/compact_experience/tail_restart_full_eval_summary.md')
...
def main() -> None:
    ...
    payload = {
        "status": "TAIL_RESTART_FULL_EVAL_SUMMARY",
        "baseline_clean_qwen_100M": BASELINE,
        "visible_leader": LEADER,
        "candidates": rows,
        "note": "AoA is measured only if the output model root contains the strict-small checkpoint ladder. Corrected AoA units: leaderboard_score=100*raw_correlation.",
    }
    _public_path('experiments/archive/compact_experience/data/tail_restart_full_eval').mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# research clean-tail restart full evaluation summary",
        "",
    ...
    for r in rows:
        ...
        lines.append(
            f"| {r['target']} | {r['endpoint']} | {r['overall']} | {r['delta_vs_clean_best']} | {r['delta_vs_visible_leader']} | {r['superglue_mean']} | {r['aoa_raw_correlation']} | {r['aoa_leaderboard_score']} | {r['submit_ready_overall']} | {s.get('BLiMP')} | {s.get('Supplement')} | {s.get('EWoK')} | {s.get('Entity')} | {s.get('COMPS')} | {s.get('GlobalPIQA')} | {s.get('Reading')} |"
        )
    _public_path('research/notes/compact_experience').mkdir(parents=True, exist_ok=True)
    NOTE.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"out": str(OUT), "note": str(NOTE), "candidates": [{"target": r["target"], "overall": r["overall"], "delta_vs_clean_best": r["delta_vs_clean_best"], "delta_vs_visible_leader": r["delta_vs_visible_leader"], "submit_ready_overall": r["submit_ready_overall"]} for r in rows]}, indent=2), flush=True)
