#!/usr/bin/env python3
"""Trainer-side core for exact-mass-matched innovation-biased WWM.

The intervention first reproduces baseline Bernoulli WWM.  On a changed row it
proposes at most one innovation group using a stable hash independent of the
baseline mask.  If that proposal was not already selected, it swaps the group
with one already-selected, non-pair ordinary group of exactly the same token
length elsewhere in the batch (ordinary rows are preferred).  Therefore batch
selected WWM group count and selected token count are exactly unchanged, not
merely matched in expectation, while source and copyable pair masks are intact.

No normalized string or category is exposed to the model.  The only model inputs
remain the inherited ``input_ids`` and ``attention_mask``; metadata controls the
boolean selection mask before the unchanged token-mean MLM loss.
"""
...
def baseline_wwm_selection(
    ...
    """Match ``masking_curriculum_trainer.apply_masking_curriculum`` WWM selection."""
    ...
    # Preserve the inherited batch-global nonempty fallback exactly.
    if select.sum() == 0:
...
def _validate_target(
    target: dict[str, Any], positions_by_gid: dict[int, tuple[int, ...]]
) -> tuple[bool, str]:
...
def innovation_biased_selection(
    ...
    """Select masks with exact same-length one-for-one swaps.

    ``row_metadata`` is parallel to the batch.  The proposal ordering depends on
    example id, group id and ``priority_salt`` only; it consumes no PyTorch RNG,
    leaving the inherited 80/10/10 corruption draws at the baseline coordinate.
    """
    ...
    for b, meta in enumerate(row_metadata):
        ...
        for target in raw_targets:
            ok, reason = _validate_target(target, pos_by_gid)
            if ok:
        ...
        # Backward-compatible conservative reconstruction for v1 drafts.
        ...
        proposals = sorted(
            valid_targets,
            key=lambda t: _stable_u64("target", priority_salt, eid, int(t["group_id"])),
        ...
        for target in proposals:
            ...
            if bool(baseline[b, list(target_pos)].all()):
                ...
                events.append({
                    "batch_row": b, "example_id": eid, "outcome": "baseline_collision",
                    "target_group_id": target_gid, "target_token_count": len(target_pos),
    ...
    # Allocate donors batch-wide. This preserves pair-side baseline masks and
    # realizes the proposal's "reduce ordinary random selections elsewhere".
    pending.sort(key=lambda x: _stable_u64(
        "pending", priority_salt, x["example_id"], x["target_gid"]
    ))
    invariant_groups = sum(
        int(bool(baseline[rb, list(p)].all()))
        for rb, ri in enumerate(row_info) for p in ri["positions"].values()
    )
    ...
    for item in pending:
        ...
        if not donors:
                ...
                events.append({
                    "batch_row": b, "example_id": eid, "outcome": "no_equal_length_donor",
                    "target_group_id": target_gid, "target_token_count": len(target_pos),
                    "target_pair_id": str(target.get("pair_id", "")),
                    "target_cue_class": str(target.get("cue_class", "")),
                })
                continue
        # Prefer ordinary rows; fallback changed-row donors must still be outside
        # every pair span. Hash ordering avoids first-row or lexical bias.
        donor_b, donor_gid = min(
            donors,
            key=lambda bg: (
                row_metadata[bg[0]] is not None,
                _stable_u64("donor", priority_salt, eid, target_gid, bg[0], bg[1]),
            ),
        )
        ...
        if (before_groups, before_tokens) != (after_groups, after_tokens):
            raise AssertionError("same-length one-for-one batch swap failed mass invariant")
        source_gids = item["pair_source"].get(str(target.get("pair_id", "")), set())
        target_positions_by_gid = row_info[b]["positions"]
        visible_source = [
            g for g in source_gids if g in target_positions_by_gid
            and not bool(biased[b, list(target_positions_by_gid[g])].all())
        ]
        fully_visible_source = bool(source_gids) and all(
            g not in target_positions_by_gid
            or not bool(biased[b, list(target_positions_by_gid[g])].any())
...
def apply_inherited_tokenwise_corruption(
    ...
    """Unchanged trainer 80/10/10 tokenwise corruption, returning its random draws."""
    ...
    if rand_tok.any():
        random_ids = torch.randint(
            0, int(vocab_size), (int(rand_tok.sum().item()),),
