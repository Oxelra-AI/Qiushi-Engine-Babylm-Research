#!/usr/bin/env python3
"""research train-time scaled residual-adapter trainer.

Thin wrapper around the exact research masking-curriculum trainer.  It changes only
model construction to AdapterDebertaV2ForMaskedLM with a fixed adapter_scale,
keeps adapter up-projection weight decay at zero as in research/103, and saves the
checkpoint-local modeling source.  Corpus, tokenizer, RNG, WWM, AdamW, and the
original 100M LR horizon remain inherited from the base trainer via CLI args.
"""
...
def _load_base():
    spec = importlib.util.spec_from_file_location("scaled_base", BASE_TRAINER)
