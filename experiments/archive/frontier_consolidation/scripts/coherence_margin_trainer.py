...
ADAPTER_MODELING = _public_path('experiments/archive/frontier_consolidation/scripts/adapter_scaled_modeling.py')
...
def save_hf_checkpoint(model, tokenizer, dst: Path) -> None:
    ...
    for src in [_public_path('experiments/archive/frontier_consolidation/scripts/coherence_margin_trainer.py'), ADAPTER_MODELING]:
        target = dst / src.name
