#!/usr/bin/env python3
"""research: evaluate a batch of adapter-scale shadow checkpoints.

Each scale shadow is a no-training checkpoint made from the research live adapter128
20M prefix with identical weights and a checkpoint-local adapter_scale multiplier.
The script runs the official-compatible cheap columns using the research wrapper.
"""
...
def eval_one(scale: float, gpu: int, out_root: Path, collate_root: Path, force: bool) -> dict:
    ...
    print(json.dumps({"event": "eval_scale_start", "scale": scale, "target": target, "gpu": gpu, "utc": now(), "cmd": cmd}), flush=True)
    t0 = time.time()
    proc = subprocess.run(cmd, cwd=str(USER_ROOT), env=env, capture_output=True, text=True, timeout=3600)
    stdout_log.write_text(proc.stdout, encoding="utf-8")
    stderr_log.write_text(proc.stderr, encoding="utf-8")
    elapsed = time.time() - t0
    rec = {"scale": scale, "tag": tag(scale), "target": target, "run_dir": str(rd), "returncode": proc.returncode, "elapsed_sec": elapsed, "stdout_log": str(stdout_log), "stderr_log": str(stderr_log)}
    ...
    rec["delta_vs_step35"]["cheap7"] = None if rec["cheap7"] is None else rec["cheap7"] - cheap7(research)
    rec["per_target"] = str(per_target)
    print(json.dumps({"event": "eval_scale_done", "scale": scale, "cheap7": rec["cheap7"], "delta_cheap7": rec["delta_vs_step35"]["cheap7"], "elapsed_sec": elapsed}, ensure_ascii=False), flush=True)
    return rec
...
def main() -> None:
    ...
    args = ap.parse_args()
    out_root = ROOT / f"eval_{args.split}"
    collate_root = ROOT / f"collate_{args.split}"
    records = []
    for s in args.scales:
        records.append(eval_one(float(s), int(args.gpu), out_root, collate_root, args.force))
    summary = {"status": "SCALE_BATCH_DONE", "gpu": args.gpu, "split": args.split, "scales": args.scales, "records": records}
    out_json = ROOT / f"scale_batch_{args.split}.json"
    out_md = ROOT / f"scale_batch_{args.split}.md"
