#!/usr/bin/env python3
"""Evaluate research train-time adapter-scale 20M arms with isolated output roots."""
...
def eval_one(key: str, gpu: int, out_root: Path, collate_root: Path, force: bool = False):
    arm = ARMS[key]
    ...
    log_dir = out_root / "logs_step104_eval_driver"
    log_dir.mkdir(parents=True, exist_ok=True)
    stdout_log = log_dir / f"{target}_stdout.log"
    stderr_log = log_dir / f"{target}_stderr.log"
    print(json.dumps({"event": "scaled_train_eval_start", "arm": key, "gpu": gpu, "cmd": cmd, "utc": now(), "out_root": str(out_root)}), flush=True)
    t0 = time.time()
    proc = subprocess.run(cmd, cwd=str(USER_ROOT), env=env, capture_output=True, text=True, timeout=2400)
    elapsed = time.time() - t0
    stdout_log.write_text(proc.stdout, encoding="utf-8")
    stderr_log.write_text(proc.stderr, encoding="utf-8")
    rec = {"arm": key, **arm, "run_dir": str(run_dir), "returncode": proc.returncode, "elapsed_sec": elapsed, "stdout_log": str(stdout_log), "stderr_log": str(stderr_log), "out_root": str(out_root)}
    ...
    if not per_target.exists():
        ...
        print(json.dumps({"event": "scaled_train_eval_missing_json", **rec}, ensure_ascii=False), flush=True)
        return rec
    ...
    rec["delta_vs_inference_scale_map"] = {c: scores[c] - SCALE_MAP[key][c] for c in CHEAP}
    rec["delta_vs_inference_scale_map"]["cheap7"] = rec["cheap7"] - cheap7(SCALE_MAP[key])
...
def summarize(records, out_base: Path):
    rows = {
        ...
        "Inference_scale1p75": {"label": "research live20M evaluated scale1.75", "scores": SCALE_MAP["scale1p75"], "cheap7": cheap7(SCALE_MAP["scale1p75"])},
        "Inference_scale2p00": {"label": "research live20M evaluated scale2.00", "scores": SCALE_MAP["scale2p00"], "cheap7": cheap7(SCALE_MAP["scale2p00"])},
    }
    for r in records:
        if r.get("scores"):
    ...
    out_json = out_base / "scaled_train_20M_summary.json"
    out_md = out_base / "scaled_train_20M_summary.md"
...
def main():
    ...
    records = [eval_one(a, args.gpu, out_root, collate_root, args.force) for a in args.arms]
    out_json, out_md = summarize(records, out_base)
