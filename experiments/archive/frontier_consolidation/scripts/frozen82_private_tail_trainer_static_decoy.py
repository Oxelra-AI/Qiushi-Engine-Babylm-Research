#!/usr/bin/env python3
"""research static-decoy variant of the frozen-82M private-tail trainer.

This contingency trainer is not the active research batch-deranged tail run.  It
preserves the verified above-41.8 scale1.75 chck_82M slow function and fresh
zero-output private residual, but in `mode=shuffled` it uses the precomputed
pair-level different-document decoy source saved in the sparse pair-data file.  That
static decoy was constructed to match `source_words`, so aligned and shuffled arms
charge identical auxiliary words whenever the same target units fire.

It exists if the observed batch-deranged tail panel is too close to interpret under
its tiny source-length/accounting mismatch; do not launch it unless the evidence
requires this cleaner control.
"""
    ...
    for bi, rec in zip(pair_batch_indices, pair_records):
        ...
        for pr in rec["pairs"]:
            ...
            units.append(AuxUnit(
                ...
                decoy_source_ids=list(pr.get("decoy_source_ids", pr["source_ids"])),
...
def source_assignment(units: list[AuxUnit], mode: str, seed: int, step: int) -> list[tuple[list[int], int]]:
    ...
    if mode == "neutral_only":
        return []
    # Static length-matched decoy from the pair-data construction.  The stored
    # decoy_source_ids can differ in token length, but the legal word count is the
    # same `source_words`, which is the charged quantity.
    return [(u.decoy_source_ids, u.source_words) for u in units]
