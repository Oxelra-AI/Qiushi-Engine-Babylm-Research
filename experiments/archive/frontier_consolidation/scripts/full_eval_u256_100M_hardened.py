#!/usr/bin/env python3
"""research: race-free full official-compatible evaluation for the U256 100M endpoint.
...
The expensive work this orchestrates is the decisive endpoint measurement after
U256's 20M screen produced +0.9179 cheap7 while retaining known EWoK/Reading risks;
only a mature nine-column endpoint can decide whether the stream-object mechanism
is a real legal SOTA route or another early redistribution.
"""
...
DEFAULT_RUN_DIR = WORKSPACE / "training/runs/eu_U256_legal16k_seed43022_100M"
DEFAULT_BASE_OUT = WORKSPACE / "data/u256_100M_full_eval_hardened"
...
def verify_endpoint_ready(run_dir: Path, endpoint: str, timeout_sec: int, skip_wait: bool) -> dict[str, Any]:
    ...
    saved = [x.get("name") for x in data.get("checkpoints", data.get("saved_checkpoints", [])) if isinstance(x, dict)]
    present = []
    missing = []
    ...
    first_loss_ok = isinstance(first_loss, (int, float)) and abs(float(first_loss) - expected_first_loss) < 1e-6
    arm_ok = data.get("arm") == "U256"
    stream_ok = data.get("stream_sha256") == "3dd19f09deeca44d6b07340e70cd15baa4b5c7bffe0bd462ff37536e470e7691"
    base_ok = data.get("base_sha256") == "215944978157394dbecf2039f2c1e1806bfcbb9701421920f78605eb58975a23"
    out = {
        "run_dir": rel(run_dir),
        "endpoint": endpoint,
        "arm": data.get("arm"),
        "arm_is_U256": arm_ok,
        "word_exposure": data.get("word_exposure", data.get("total_charged_words")),
        "total_steps_executed": data.get("total_steps_executed", data.get("actual_training_steps")),
        "loss_first": first_loss,
        "expected_first_loss": expected_first_loss,
        "loss_first_matches_U256_20M_prefix": first_loss_ok,
        ...
        "base_sha256_matches_step35_pool": base_ok,
        "stream_sha256": data.get("stream_sha256"),
        "stream_sha256_matches_step35_stream": stream_ok,
        "active_tokens_per_epoch": data.get("raw_tokens_per_epoch"),
    }
    if missing:
        raise RuntimeError({"incomplete_aoa_ladder": out})
    if not (first_loss_ok and arm_ok and stream_ok and base_ok):
        raise RuntimeError({"unexpected_U256_endpoint_metrics": out})
    return out
...
def stage_predictions(base_out: Path, run_dir: Path, target: str, endpoint: str, copy: bool) -> dict[str, Any]:
    ...
    merged_payload = {
        "target": target,
        "description": "U256 faithful experience-utilization 100M endpoint, official-compatible predictions staged from race-free split jobs",
        "family": "u256_faithful_experience_utilization_100M_full_eval",
        "run_dir": rel(run_dir),
...
def summarize(base_out: Path, target: str, endpoint_ready: dict[str, Any], eval_rows: list[dict[str, Any]], stage_summary: dict[str, Any], collation: dict[str, Any]) -> dict[str, Any]:
    ...
    summary = {
        "status": "U256_100M_FULL_EVAL_HARDENED",
        "created_utc": now(),
        "target": target,
    ...
    out_json = base_out / "summary" / "u256_100M_full_eval_hardened_summary.json"
    out_md = base_out / "summary" / "u256_100M_full_eval_hardened_summary.md"
    write_json(out_json, summary)
    lines = [
        "# research U256 100M Full Official-Compatible Evaluation",
        "",
        f"Overall: **{overall:.6f}** (margin vs 41.8: **{margin:+.6f}**)",
        f"cheap7: **{cheap7:.6f}** (delta vs research: **{cheap7 - reference_100M_REF['cheap7']:+.6f}**)",
        "",
        "| Column | U256 100M | research 100M | Delta |",
        "|---|---:|---:|---:|",
    ]
...
def run_full(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.collate_dry_run:
        out = {"status": "U256_100M_COLLATE_DRYRUN", "endpoint_ready": endpoint_ready, "eval_rows": eval_rows, "stage_summary": stage_summary, "collation": collation}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Race-free full evaluation for U256 100M endpoint")
