#!/usr/bin/env python3
"""research: materialize a distribution-matched MAX dose with coherent row holdout.

Repairs the research prototype materializer for the decisive dose experiment:
* the inherited 1x compact block is preserved as the inner prefix of the pair set;
* the increment is selected by research distribution matching;
* clean-Qwen base displacement uses intact non-Qwen 160-word row holdout, as in
  the research legal 1x anchor, not word-level stream shuffling;
* view and hash-rotated-repeat arms share row lengths, common filler, topup rows,
  and identical per-pass row order; only the second view differs;
* the research compliant16k_reinvest10M tokenizer is held fixed for later training.

The output arms are mechanism instruments, not new leaderboard submissions: their
documented text spans the old tokenizer-fitting/pretraining pool plus additional
generated FineWeb source/view material.
"""
...
PAIRS_DEFAULT = WORKSPACE / "data/dose_distribution_select/selected_matched_max_pairs.jsonl"
OLD_HELDOUT_DEFAULT = WORKSPACE / "data/density_cleanqwen_overlay_medium_riskhard/heldout_cleanqwen_rows.jsonl"
OUT_DIR_DEFAULT = WORKSPACE / "data/dose_2p64x_rowholdout_pools"
...
def main() -> None:
    ...
    view_rows = pack_pair_rows(pairs, True, "compact_view_dose2p64x_matched_rowholdout", 970000)
    repeat_rows = pack_pair_rows(pairs, False, "compact_repeat_dose2p64x_matched_rowholdout", 970000)
    ...
    while rem > 0:
        ...
        rem -= L
    topup = stream_rows(holdout_stream, topup_lengths, "neutral_cleanqwen_topup_dose2p64x", 1_070_000) if topup_lengths else []
    clean_changed = stream_rows(holdout_stream, pair_lengths + topup_lengths, "cleanqwen_lengthmatched_dose2p64x", 1_170_000)
    filler_rows = [Row(r.text, r.words, r.example_id, r.source) for r in filler]
    # Shuffle the common filler once; the same object/order is appended to all arms.
    random.Random(args.seed + 12345).shuffle(filler_rows)
    arms = {
        "cleanqwen_lengthmatched_dose2p64x": clean_changed + filler_rows,
        "compact_repeat_dose2p64x": repeat_rows + topup + filler_rows,
        "compact_view_dose2p64x": view_rows + topup + filler_rows,
    }
    ...
    for name, rows in arms.items():
        ...
        if args.write_training and name != "cleanqwen_lengthmatched_dose2p64x":
    ...
    payload = {
        "status": "MATCHED_MAX_ROWHOLDOUT_POOLS_MATERIALIZED",
        "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "scientific_purpose": "MAX dose mechanism instrument for fixed-budget BabyLM data-efficient learning: amplify the fraction of the 10M pool converted from clean-Qwen filler to source+compact-view packets while keeping tokenizer, architecture, optimizer, seeds, and row-holdout construction compatible with the 1x anchor.",
        "inputs": {
        ...
        "base_split": {
            ...
            "old_1x_heldout_subset_of_max": old_subset,
        ...
        "audit": {
            ...
            "view_repeat_suffix_identical_after_changed_block": all(row_to_record(a) == row_to_record(b) for a, b in zip(arms["compact_view_dose2p64x"][len(view_rows):], arms["compact_repeat_dose2p64x"][len(repeat_rows):])),
        ...
        "sha256": hashes,
        "boundary": "MAX arms are scientific mechanism instruments whose documented text spans more than one 10M pool; they are not candidate leaderboard uploads. Stable-family evaluation only is intended here.",
        "elapsed_sec": round(time.time() - t0, 2),
    }
    meta_path = out_dir / "dose2p64x_rowholdout_metadata.json"
    meta_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = [
        "# research matched MAX row-holdout dose pools",
        "",
        f"Selected pair words: {pair_words:,}; row-holdout changed block: {changed_budget:,} words; dose {dose_multiple:.4f}x; topup {topup_words} words.",
        f"Arms have exact 10M words and identical row-length sequences: {payload['audit']['row_length_sequence_identical_all_arms']}.",
        f"Old 1x heldout subset of MAX heldout: {old_subset}; overlap rows {payload['base_split']['old_1x_heldout_overlap_rows']} / {len(old_held_ids)}.",
    ...
    (out_dir / "dose2p64x_rowholdout_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({
