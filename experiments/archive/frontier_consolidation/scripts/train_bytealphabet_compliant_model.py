#!/usr/bin/env python3
"""research: train frozen BabyLM models with the legal byte-alphabet compliant tokenizer.

This launcher is a surgical repair of research after research CPU audits found that the
research same-pool tokenizer maps newline and some byte symbols to <unk> in likely
scored official text (especially Supplement dialogue/QA strings).  The tokenizer
used here is trained on exactly the same allowed 10M reinvest pool, with the same
16k byte-level BPE setup, but with BpeTrainer(initial_alphabet=ByteLevel.alphabet()).

Everything else stays frozen relative to the original compact_view_reinvest endpoint:
corpus, architecture, random seeds, WWM probability, optimizer, sequence length,
batch size, checkpoint cadence, and 100M word exposure.
"""
...
TOKENIZER = WORKSPACE / "data" / "compliant_tokenizer_bytealphabet"
EXPECTED_TOKENIZER_SHA = "b2b317e655a96f2c14eb559ce9fa7573f180904d84819cacfb5f61174cf355cf"
...
ARMS: dict[str, dict[str, Any]] = {
    "reinvest": {
        ...
        "run_name": "bytealphatok_reinvest_seed43022",
        "example_label": "bytealphatok_cleanqwen_fineweb_compact_view_reinvest",
        "scientific_role": "submission-candidate endpoint recovery under the legal same-pool byte-alphabet compliant tokenizer",
    },
    "clean_qwen": {
        ...
        "run_name": "bytealphatok_cleanqwen_seed43022",
        "example_label": "bytealphatok_qwen_clean_aligned",
        "scientific_role": "matched clean-Qwen fixed-tokenizer control under the same byte-alphabet tokenizer; scientific control only, not independent submission candidate",
    },
}

SEED_CFG = {"seed": "43", "extra_init_seed": "43022", "train_rng_seed": "43023"}
...
def tokenizer_pool_words(meta: dict[str, Any]) -> int | None:
    td = meta.get("training_data", {})
    if "pool_words" in td:
        return int(td["pool_words"])
    if "words" in td:
        return int(td["words"])
    return None
...
def preflight(arm: str, run_dir: pathlib.Path, check_hash: bool, count_words: bool) -> dict[str, Any]:
    ...
    required = {
        ...
        "tokenizer_metadata": TOKENIZER / "tokenizer_metadata.json",
    ...
    if tokenizer_pool_words(tok_meta) != 10_000_000:
        raise RuntimeError(f"Tokenizer metadata does not show 10M training data: {tok_meta}")
    if tok_meta.get("training_data", {}).get("pool_sha256") != EXPECTED_POOL_SHA:
        raise RuntimeError(f"Tokenizer pool SHA mismatch in metadata: {tok_meta.get('training_data')}")
    tok_info = tok_meta.get("tokenizer", {})
    if int(tok_info.get("vocab_size", -1)) != 16_384:
        raise RuntimeError(f"Tokenizer vocab not 16384: {tok_info}")
    if tok_info.get("missing_bytelevel_alphabet") not in ([], None):
        raise RuntimeError(f"Byte alphabet not complete: {tok_info.get('missing_bytelevel_alphabet')}")
    special_ids = tok_info.get("special_token_ids", {})
    expected_special = {"<unk>": 0, "<s>": 1, "</s>": 2, "<pad>": 3, "<mask>": 4}
    if special_ids and special_ids != expected_special:
        raise RuntimeError(f"Unexpected special token ids: {special_ids}")
    ...
    return {
        "status": "BYTEALPHATOK_TRAIN_PREFLIGHT_OK",
        "arm": arm,
        ...
        "recipe": RECIPE,
        "repair_reason": "research CPU scored-text audits found research tokenizer produced <unk> for newline in Supplement dialogue/QA strings; this tokenizer uses the same 10M pool but explicit ByteLevel.alphabet() and has zero scored-text <unk> in the research audit.",
    }
...
def build_command(arm: str, run_dir: pathlib.Path, max_word_exposure: int) -> list[str]:
    ...
    return [
        ...
        "--tokenizer_label", "compliant16k_reinvest10M_bytealphabet",
        "--hidden_size", "480",
        "--n_layer", "8",
        "--n_head", "8",
        "--ffn_mult", "4",
...
def main() -> None:
    ...
    env["TMPDIR"] = f"/tmp/q_frontier_consolidation_step041_{args.arm}_{int(time.time())}"
    ...
    result: dict[str, Any] = {
        "status": "BYTEALPHATOK_TRAIN_FINISHED" if proc.returncode == 0 else "BYTEALPHATOK_TRAIN_FAILED",
        "arm": args.arm,
        "gpu": args.gpu,
