#!/usr/bin/env python3
"""research: train/dry-run the MAX-dose permuted-companion DeBERTa control.

The data arm is a correspondence-breaking control: same MAX source multiset,
same accepted compact-rewrite multiset, same row geometry, same suffix/topup,
same 100M row order formula, but each source is paired with another source's
compact rewrite excluding same-row and same-document donors.  This script only
runs training when explicitly invoked without --dry-run; policy is to launch it
only after the second-basin MAX aligned view/repeat readout reproduces the Entity
carrier strongly enough to make aligned-versus-permuted decisive.
"""
...
POOL_DIR = WS / 'data/dose_2p64x_permuted_companion_rowholdout_pools'
META_PATH = POOL_DIR / 'permuted_companion_rowholdout_metadata.json'
STREAM_PATH = POOL_DIR / 'compact_permuted_view_dose2p64x_100M.jsonl'
RUNS_DIR = WS / 'training/runs'
DEFAULT_RUN_DIR = RUNS_DIR / 'full_p2c_c2p_abs_permuted_view_dose2p64x_matched_rowholdout_deberta100M_seed43022'
...
STREAM_LABEL = 'compact_permuted_view_dose2p64x_matched_rowholdout'
...
def count_jsonl(path: pathlib.Path) -> dict[str, Any]:
    ...
    return {
        ...
        'source_prefix_counts': {
            'compact_permuted_view': sum(v for k, v in by_source.items() if k.startswith('compact_permuted_view')),
...
def summarize_metrics(metrics_path: pathlib.Path) -> dict[str, Any]:
    ...
    return {
        ...
        'all_expected_checkpoints_present': len(saved) == 10 and saved[0].get('name') == 'chck_10M' and saved[-1].get('name') == 'chck_100M' if saved else False,
    }
...
def preflight(args: argparse.Namespace, run_dir: pathlib.Path) -> dict[str, Any]:
    ...
    if meta.get('status') != 'PERMUTED_COMPANION_MAX_ROWHOLDOUT_MATERIALIZED':
        raise RuntimeError(f'unexpected metadata status: {meta.get("status")}')
    audit = meta.get('audit') or {}
    required = {
        'word_total_exact_10M': True,
        'row_length_sequence_matches_max_view': True,
        'suffix_exactly_reused_from_max_view_after_changed_block': True,
        'assignment_is_permutation': True,
        'companion_text_multiset_identical_to_max_view': True,
        'all_sources_receive_different_pair_rewrite': True,
        'same_doc_assignment_count': 0,
        'same_row_assignment_count': 0,
        'donor_doc_in_target_row_docset_count': 0,
        'all_row_companion_word_sums_preserved': True,
    }
    failed = {k: {'expected': v, 'observed': audit.get(k)} for k, v in required.items() if audit.get(k) != v}
    if failed:
        raise RuntimeError(f'permuted metadata audit not clean: {failed}')
    expected_stream_sha = str((meta.get('sha256') or {}).get('compact_permuted_view_dose2p64x_100M.jsonl') or '')
    ...
    return {
        'status': 'PERMUTED_COMPANION_TRAIN_PREFLIGHT_OK',
        'created_utc': now(),
        'scientific_purpose': 'Train the correspondence-broken MAX compact companion arm so aligned-minus-permuted isolates whether same-window source-view correspondence, not merely the compact rewrite text multiset, carries the Entity/state-tracking dose effect.',
        'predeclared_launch_condition': 'Launch only if the active second-basin MAX DeBERTa view/repeat EWoK+Entity readout reproduces a substantial Entity-positive aligned effect; otherwise this arm remains a materialized control and no H100 should be spent.',
        'data_arm': 'permuted_companion',
        'run_dir': rel(run_dir),
        'pool_metadata': rel(META_PATH),
        'dose': (meta.get('inputs') or {}).get('status'),
        ...
        'expensive_work_role': 'If launched under the condition above, this one H100 run directly decides whether the MAX aligned-view effect requires source-to-own-view correspondence. Aligned≫permuted supports record addressability; aligned≈permuted redirects the principle toward compact companion text or token/regularization effects.',
...
def main() -> None:
    ...
    env['TMPDIR'] = f'/tmp/q_frontier_consolidation_step270_permuted_{int(time.time())}'
    ...
    with stdout_path.open('w', encoding='utf-8') as out, stderr_path.open('w', encoding='utf-8') as err:
        out.write(json.dumps({'event': 'permuted_companion_launcher_start', 'utc': started, 'gpu': args.gpu}) + '\n')
        out.flush()
    ...
    result = {
        'status': 'PERMUTED_COMPANION_TRAIN_FINISHED' if proc.returncode == 0 else 'PERMUTED_COMPANION_TRAIN_FAILED',
