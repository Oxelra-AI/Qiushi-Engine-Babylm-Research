#!/usr/bin/env python3
"""Frozen-slow + fresh-private residual DeBERTa-v2 MLM.

This model preserves the verified first-stage residual adapter function as the slow path and
adds a second private adapter after each completed encoder layer.  The inherited adapter
remains in the state-dict under `.adapter.*` and uses `config.adapter_scale`; the
new trainable pathway is under `.private_adapter.*` and uses
`config.private_adapter_scale`.
...
non-private parameters and update only `.private_adapter.*` while keeping the inherited
...
_THIS_FILE = _public_path('experiments/archive/functional_learning/data/representative_model_packages/Qiushi-Engine-Frontier-Advancement/modeling_frozen_slow_private_debertav2.py')
_REMOTE_MODULE = "modeling_frozen_slow_private_debertav2"
_AUTO_MAP = {
    "AutoModelForMaskedLM": f"{_REMOTE_MODULE}.FrozenSlowPrivateDebertaV2ForMaskedLM",
    "AutoModel": f"{_REMOTE_MODULE}.FrozenSlowPrivateDebertaV2Model",
}


def _set_public_auto_map(config: DebertaV2Config, architecture: str) -> None:
    """Keep saved configs reloadable from the public modeling filename."""
    config.architectures = [architecture]
    config.auto_map = dict(_AUTO_MAP)
...
class FrozenSlowPrivateDebertaV2Model(DebertaV2Model):
    ...
    Layer computation is identical to FrozenSlowPrivateDebertaV2ForMaskedLM:
        base = output(intermediate(attention(h)), attention(h))
        slow = base + adapter(base)           # inherited first-stage function
    ...
    def __init__(self, config: DebertaV2Config):
        ...
        _set_public_auto_map(config, self.__class__.__name__)
    ...
    def save_pretrained(self, save_directory, *args, **kwargs):
        _set_public_auto_map(self.config, self.__class__.__name__)
        super().save_pretrained(save_directory, *args, **kwargs)
        dst = Path(save_directory) / f"{_REMOTE_MODULE}.py"
...
class FrozenSlowPrivateDebertaV2ForMaskedLM(DebertaV2ForMaskedLM):
    ...
    def __init__(self, config: DebertaV2Config):
        super().__init__(config)
        # Existing slow-path adapter fields are loaded from the inherited first-stage
        # residual adapter endpoint and should be adapter_bottleneck=128, adapter_scale=1.75.
        config.adapter_bottleneck = int(getattr(config, "adapter_bottleneck", 128))
        config.adapter_activation = getattr(config, "adapter_activation", "gelu")
        config.adapter_enabled = bool(getattr(config, "adapter_enabled", True))
        config.adapter_scale = float(getattr(config, "adapter_scale", 1.75))
