...
class RotaryEmbedding(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        with torch.no_grad():
            self.cos.copy_(freqs.cos())
            self.sin.copy_(freqs.sin())
...
def _select_flex_backend(device: torch.device) -> str:
    ...
    # Runtime compatibility patch: this host's PyTorch exposes FlexAttention FLASH
    # but lacks the CUTE flash library required by torch.compile lowering.
    # TRITON is available and preserves the same public model/optimizer/objective
    # semantics while changing only the compiled attention kernel backend.
    return "TRITON"
