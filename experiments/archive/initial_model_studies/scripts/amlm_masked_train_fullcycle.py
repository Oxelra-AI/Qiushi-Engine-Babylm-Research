...
from amlm_trainer import AMLMState, apply_masking_amlm
...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--amlm_mode", choices=["none", "hard"], default="hard")
    p.add_argument("--amlm_target_updates", type=int, default=48, help="Normalize AMLM updates by training progress; window=max(3, round(total_steps/target_updates))")
    p.add_argument("--amlm_window", type=int, default=0, help="Override AMLM update window; 0 derives from total_steps/target_updates")
    p.add_argument("--mask_prob_start", type=float, default=0.4)
...
def main() -> None:
    ...
    amlm_window = args.amlm_window if args.amlm_window and args.amlm_window > 0 else max(3, round(total_steps / max(1, args.amlm_target_updates)))
    amlm_state = AMLMState(
        vocab_size=len(tokenizer), mode=args.amlm_mode, window=amlm_window,
        mask_prob_start=args.mask_prob_start, mask_prob_end=args.mask_prob_end,
        lambda_smooth=args.amlm_lambda, total_steps=total_steps,
    )
    amlm_update_records: list[dict] = []
    ...
    with log_path.open("w", encoding="utf-8") as logf:
        for step, batch in enumerate(loader, 1):
            ...
            masked_inputs, labels = apply_masking_amlm(input_ids, attention_mask, word_group, tokenizer, args.mask_mode, amlm_state, gen)
            optim.zero_grad(set_to_none=True)
            out_model = model(input_ids=masked_inputs, attention_mask=attention_mask, labels=labels)
            loss = out_model.loss
            if getattr(out_model, "logits", None) is not None:
                amlm_state.record_batch(input_ids, labels, out_model.logits.detach())
            if loss is None:
            ...
            optim.step()
            sched.step()
            updated_amlm = amlm_state.maybe_update_weights()
            if updated_amlm:
                weights = amlm_state.token_weights
                amlm_update_records.append({
                    "step": step,
                    "cumulative_word_exposure_before_batch_add": cumulative_words,
                    "base_prob": amlm_state.get_current_base_prob(),
                    "weight_mean": float(weights.mean().item()),
                    "weight_min": float(weights.min().item()),
                    "weight_max": float(weights.max().item()),
                    "n_updates": amlm_state.n_updates,
                })
            ...
            rec = {"step": step, "loss": loss_float, "lr": float(sched.get_last_lr()[0]),
                   ...
                   "seq_len": cur_len, "masked_tokens": n_pred, "amlm_base_prob": amlm_state.get_current_base_prob(),
                   "amlm_updates": amlm_state.n_updates, "elapsed_sec": time.time() - start_time}
    ...
    metrics = {
        ...
        "mask_mode": args.mask_mode,
        "mask_prob": args.mask_prob,
        "amlm_mode": args.amlm_mode,
        "amlm_target_updates": args.amlm_target_updates,
        "amlm_window": amlm_window,
        "mask_prob_start": args.mask_prob_start,
        "mask_prob_end": args.mask_prob_end,
        "amlm_lambda": args.amlm_lambda,
        "amlm_actual_updates": amlm_state.n_updates,
        "amlm_update_records": amlm_update_records,
        "amlm_token_weight_mean": float(amlm_state.token_weights.mean().item()),
        "amlm_token_weight_min": float(amlm_state.token_weights.min().item()),
        "amlm_token_weight_max": float(amlm_state.token_weights.max().item()),
