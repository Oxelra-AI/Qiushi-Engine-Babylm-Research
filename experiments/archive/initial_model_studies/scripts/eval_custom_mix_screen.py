#!/usr/bin/env python3
"""research: official-compatible two-arm evaluation for custom-corpus 10M screen.

Compares:
  - official_only: exact 10M official text, JSONL packed/shuffled control
  - custom_mix: 65% official + 24.5% SimpleWiki + 10.5% GEM simplified

Decisive comparison: custom_mix minus official_only on Entity/EWoK/COMPS while
checking Supplement/Reading preservation. GlobalPIQA-only movement is not enough.
"""
...
OUT_JSON=ROOT/'data/custom_mix_screen_scores.json'
LOG=(ROOT.parents[2] / 'research/notes/initial_model_studies/custom_mix_screen_eval.log')

MODELS={
 'official_only': ROOT/'training/runs/official_only_10M/hf_model/chck_10M',
 'custom_mix': ROOT/'training/runs/custom_mix_10M/hf_model/chck_10M',
}
...
def main():
    ...
    env=env_setup(); payload={'status':'CUSTOM_MIX_SCREEN','models':{},'notes':{'decision':'custom_mix must improve Entity/EWoK/COMPS or broad Overall-relevant columns while preserving Supplement/Reading; GlobalPIQA-only movement is insufficient'}}
    with LOG.open('w',encoding='utf-8') as logf:
        for name,model in MODELS.items():
            outdir=model.parent.parent/f'eval_step220_{name}'; outdir.mkdir(parents=True,exist_ok=True)
            scores={}; reports={}
            for col,task,data,dataset in TASKS:
                run([sys.executable,'-m','evaluation_pipeline.sentence_zero_shot.run','--model_path_or_name',str(model.resolve()),'--backend','mlm','--task',task,'--data_path',str(data.resolve()),'--save_predictions','--revision_name',f'step220_{name}','--batch_size','128' if col!='ewok' else '64','--output_dir',str(outdir.resolve())],env,logf)
            ...
            run([sys.executable,'-m','evaluation_pipeline.reading.run','--model_path_or_name',str(model.resolve()),'--backend','mlm','--data_path',str(reading.resolve()),'--revision_name',f'step220_{name}','--output_dir',str(outdir.resolve())],env,logf)
    ...
    o=payload['models']['official_only']['scores']; c=payload['models']['custom_mix']['scores']
    keys=sorted(set(o)&set(c))
    payload['deltas']={'custom_minus_official':{k:c[k]-o[k] for k in keys}}
