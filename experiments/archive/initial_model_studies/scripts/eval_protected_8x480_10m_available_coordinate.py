...
RUN = ROOT / 'training/runs/babylm_fullcycle_debertav2_8x480_wwm_seed42_100M_b256'
MODEL = (RUN / 'hf_model/chck_10M').resolve()
OUTDIR = (RUN / 'eval_results_chck10M_available_direct').resolve()
OUT_JSON = ROOT / 'data/protected_8x480_10m_available_coordinate.json'
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/protected_8x480_10m_available_coordinate.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/protected_8x480_10m_eval.log')
BACKEND = 'mlm'
REV = 'direct_chck_10M'
TASKS = [
...
def main():
    ...
    with LOG.open('a', encoding='utf-8') as logf:
        for col, task, data_path, dataset_name in TASKS:
            ...
            if not candidates:
                raise RuntimeError(f'report not found for {col}')
            report=sorted(candidates, key=lambda p: str(p))[-1]
        ...
        reading_report=sorted(candidates, key=lambda p: str(p))[-1]
    ...
    payload={'status':'PROTECTED_8X480_10M_AVAILABLE_COORDINATE','run':str(RUN),'model_path_direct':str(MODEL),'backend':BACKEND,'revision_label':REV,
             'training_core':{k:metrics.get(k) for k in ['parameter_count','embedding_parameter_count','non_embedding_parameter_count','word_exposure','actual_training_steps','lr_schedule_total_steps','loss_first','loss_last','masked_tokens_total','masked_tokens_per_whitespace_word','hidden_size','n_layer','n_head','intermediate_size','vocab_size']},
             'checkpoint_note':'This evaluates chck_10M from the protected 100M run; run-level metrics describe the full 100M run, not only the checkpoint.',
             ...
             'note':'Direct checkpoint path evaluation avoids the local-root revision selection bug and matches the research S1 task set.'}
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True); OUT_JSON.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+'\n', encoding='utf-8')
    lines=['# research — protected 8x480 DeBERTa WWM chck_10M available coordinate','',f'Evidence JSON: `{OUT_JSON}`','',f'Direct checkpoint: `{MODEL}`','', '| column/task | score |','|---|---:|']
    ...
    lines += ['', 'This is the matched 10M comparison point for S1 12x384; it is not the protected 100M endpoint.']
    OUT_NOTE.write_text('\n'.join(lines)+'\n', encoding='utf-8')
