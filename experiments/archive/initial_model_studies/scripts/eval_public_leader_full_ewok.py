...
RUN = ROOT / 'data/leader_package_revision_124/model_side'
MODEL = (RUN / 'local').resolve()
EVAL_OUT = ROOT / 'data/leader_package_revision_124/model_side/eval_results_full_ewok_word_tokenize_direct'
SUMMARY = ROOT / 'data/public_leader_ewok_word_tokenize_filter_summary.json'
SCORE_JSON = ROOT / 'data/public_leader_full_ewok_word_tokenize_score.json'
NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/public_leader_full_ewok_word_tokenize_score.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/public_leader_full_ewok_eval.log')
...
def main():
    ...
    payload={'status':'PUBLIC_LEADER_FULL_EWOK_WORD_TOKENIZE','model':'Public leader go76dof/wwm_curriculum_simplification_40k locally staged checkpoint','model_path_direct':str(MODEL),'backend':'mlm','ewok_full_score':score,'filter_summary':str(SUMMARY),'report':str(report),'predictions':str(pred),'eval_output_dir':str(EVAL_OUT),'elapsed_sec':time.time()-t0,'warning':'Full local EWoK generated from provided gated parquet with official nltk.word_tokenize resources and official vocab filter.'}
    SCORE_JSON.parent.mkdir(parents=True, exist_ok=True); SCORE_JSON.write_text(json.dumps(payload, indent=2, ensure_ascii=False)+'\n', encoding='utf-8')
    NOTE.write_text('\n'.join(['# research — public leader full EWoK with official word_tokenize','',f'Evidence JSON: `{SCORE_JSON}`',f'Filter summary: `{SUMMARY}`',f'Report: `{report}`','',f'Full EWoK score: **{score:.2f}**',f'Filtered items: {total_filtered}; swapped JSONL lines: {total_lines}','', 'This uses the local EWoK parquet, official BabyLM vocab filter, and NLTK `word_tokenize` with the local `data/nltk_data` resources.'])+'\n', encoding='utf-8')
