...
OUT_JSON = ROOT / 'data/s1_10m_available_coordinate.json'
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/s1_10m_available_coordinate.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/s1_10m_eval.log')
BACKEND = 'mlm'
REV = 'direct_chck_10M'
TASKS = [
...
def main():
    ...
    payload={'status':'S1_10M_AVAILABLE_COORDINATE','run':str(RUN),'model_path_direct':str(MODEL),'backend':BACKEND,'revision_label':REV,
             ...
             'note':'Direct checkpoint path evaluation avoids the local-root revision selection bug; S1 is legal leader-shape isolation, not leader reproduction.'}
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True); OUT_JSON.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+'\n', encoding='utf-8')
    lines=['# research — S1 leader-shape 10M available coordinate','',f'Evidence JSON: `{OUT_JSON}`','',f'Direct checkpoint: `{MODEL}`','', '| column/task | score |','|---|---:|']
    ...
    lines += ['', 'This is a 10M architecture-shape screen on official data with baseline16k tokenizer and flat WWM; exact leader data remains gated.']
    OUT_NOTE.write_text('\n'.join(lines)+'\n', encoding='utf-8')
