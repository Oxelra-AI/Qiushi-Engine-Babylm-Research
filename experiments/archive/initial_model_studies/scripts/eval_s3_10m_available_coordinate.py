...
OUT_JSON = ROOT / 'data/s3_10m_available_coordinate.json'
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/s3_10m_available_coordinate.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/s3_10m_eval.log')
BACKEND = 'mlm'
REV = 'direct_chck_10M'
TASKS = [
...
def main():
    ...
    payload={'status':'S3_10M_AVAILABLE_COORDINATE','run':str(RUN),'model_path_direct':str(MODEL),'backend':BACKEND,'revision_label':REV,
             ...
             'note':'Direct checkpoint path evaluation; S3 is legal official40k-under-12x384 isolation on official corpus, not leader reproduction.'}
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True); OUT_JSON.write_text(json.dumps(payload,indent=2,ensure_ascii=False)+'\n', encoding='utf-8')
    lines=['# research — S3 official40k-under-12x384 10M available coordinate','',f'Evidence JSON: `{OUT_JSON}`','',f'Direct checkpoint: `{MODEL}`','', '| column/task | score |','|---|---:|']
    ...
    lines += ['', 'This is a 10M legal official40k-under-12x384 screen on official data with flat WWM; exact leader data remains gated and this tokenizer is official-corpus ByteLevel-BPE, not the leader SentencePiece tokenizer.']
    OUT_NOTE.write_text('\n'.join(lines)+'\n', encoding='utf-8')
