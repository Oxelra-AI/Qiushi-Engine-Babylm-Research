#!/usr/bin/env python3
"""research matched 1M training discriminator for same-entity deranged pair arms.

Trains four arms on the research corrected deranged 1M data, then evaluates each
on the fast profile. Primary contrast: true_pair_adjacent - hard_negative_same_entity
on Entity Tracking fast and EWoK fast. Same research configuration.
"""
...
RUN_DIRS = {a: ROOT/f'training/runs/step291_{a}_debertav2_8x480_1M_b128_seed42' for a in ARMS}
...
def train_arm(arm, env, logf):
    ...
    cmd = [PYBIN, str(TRAIN),
           ...
           '--example_jsonl_label', f'step291_{arm}_1M_b128',
...
def profile_arm(arm, env, logf):
    ...
    outdir = run_dir/'eval_step291_fast_profile'
    scores = {}; reports = {}
    ...
    return {'run_dir': str(run_dir), 'scores': scores, 'reports': reports}
...
def main():
    ...
    with LOG.open('a', encoding='utf-8') as logf:
        logf.write(f'\n===== research discriminator start {time.ctime()} =====\n')
        for a in ARMS:
            logf.write(f'\n--- training {a} ---\n'); logf.flush()
    ...
    tp = rows['true_pair_adjacent']['scores']
    hn = rows['hard_negative_same_entity']['scores']
    oo = rows['orig_only']['scores']
    sh = rows['shuffled_pair_adjacent']['scores']
    ...
    payload = {'status':'PAIR_DISCRIMINATOR_1M_PROFILE','arms':rows,
               ...
               'design_note':'Deranged same-entity control: true and hard arms share identical original and simplification multisets; only adjacency correspondence differs. Primary contrast is true_minus_hardneg on entity_tracking_fast and ewok_fast.'}
