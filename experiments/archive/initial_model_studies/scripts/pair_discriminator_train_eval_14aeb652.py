#!/usr/bin/env python3
"""research matched 1M training discriminator on explicit free GPU.

Same experiment as research, fresh output paths after research GPU0 OOM.
Primary contrast: true_pair_adjacent - hard_negative_same_entity on Entity/EWoK.
No data/model/objective changes.
"""
...
RUN_DIRS = {a: ROOT/f'training/runs/step293_{a}_debertav2_8x480_1M_b128_seed42' for a in ARMS}
OUT_JSON = ROOT/'data/pair_discriminator_1m_profile.json'
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/pair_discriminator_1m_profile.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/pair_discriminator_1m.log')
TASKS = [
...
def setup_env():
    ...
    # Preserve caller's CUDA_VISIBLE_DEVICES; require explicit caller setting for research.
...
def run(cmd, env, logf, cwd=None):
    ...
    tail = p.stdout[-3000:]
    print(tail, flush=True); logf.write(p.stdout+f'\n[returncode={p.returncode}]\n'); logf.flush()
    if p.returncode != 0:
        raise RuntimeError(f'failed {p.returncode}: {line}\n{p.stdout[-8000:]}')

def train_arm(arm, env, logf):
    run_dir = RUN_DIRS[arm]
    jsonl = DATA/f'{arm}_1000000w.jsonl'
    if not jsonl.exists():
        raise FileNotFoundError(jsonl)
    if run_dir.exists() and (run_dir/'hf_model'/'chck_1M'/'config.json').exists():
        logf.write(f'Existing complete checkpoint for {arm}; skipping training.\n'); logf.flush()
        return
    cmd = [PYBIN, str(TRAIN),
           '--output_dir', str(run_dir),
           '--example_jsonl', str(jsonl),
           '--example_jsonl_label', f'step293_{arm}_1M_b128',
...
def load_metrics(run_dir):
    ...
    return {
        ...
        'batch_size': 128,
        'truncated_fraction': (m.get('tokenization_coupling_summary') or {}).get('truncated_example_fraction'),
        'tokens_per_word_kept': (m.get('tokenization_coupling_summary') or {}).get('kept_tokens_per_whitespace_word'),
...
def profile_arm(arm, env, logf):
    ...
    for task_name, task, data_path, ds_name in TASKS:
        run([PYBIN,'-m','evaluation_pipeline.sentence_zero_shot.run',
             '--model_path_or_name', str((run_dir/'hf_model').resolve()),
             '--backend','mlm','--task',task,'--data_path',data_path,'--save_predictions',
             '--revision_name','chck_1M','--batch_size','64','--output_dir',str(outdir.resolve())],
    ...
    run([PYBIN,'-m','evaluation_pipeline.reading.run',
         '--model_path_or_name', str((run_dir/'hf_model').resolve()),
         '--backend','mlm','--data_path','evaluation_data/fast_eval/reading/reading_data.csv',
         '--revision_name','chck_1M','--output_dir',str(outdir.resolve())],
        env, logf, cwd=str(STRICT))
...
def main():
    ...
    if os.environ.get('CUDA_VISIBLE_DEVICES') is None:
        print('WARNING: CUDA_VISIBLE_DEVICES not set; research intended to be launched with CUDA_VISIBLE_DEVICES=1', flush=True)
    meta = json.loads(META.read_text())
    for a in ARMS:
        assert meta['arms'][a]['words'] == 1000000, f'{a} not exact 1M'
    rows = {}
    with LOG.open('a', encoding='utf-8') as logf:
        logf.write(f'\n===== research discriminator start {time.ctime()} cuda={os.environ.get("CUDA_VISIBLE_DEVICES")} =====\n')
        for a in ARMS:
            logf.write(f'\n--- training {a} ---\n'); logf.flush()
            train_arm(a, env, logf)
        for a in ARMS:
            logf.write(f'\n--- profiling {a} ---\n'); logf.flush()
            rows[a] = profile_arm(a, env, logf)
    ...
    primary = {k: round(tp[k]-hn[k], 4) for k in COLS}
    vs_orig = {k: round(tp[k]-oo[k], 4) for k in COLS}
    vs_shuf = {k: round(tp[k]-sh[k], 4) for k in COLS}
    payload = {'status':'PAIR_DISCRIMINATOR_1M_PROFILE','arms':rows,
               'true_minus_hardneg':primary,'true_minus_orig':vs_orig,'true_minus_shuffled':vs_shuf,
               'elapsed_sec':time.time()-t0,
               'design_note':'Deranged same-entity control: true and hard arms share identical original and simplification multisets; only adjacency correspondence differs. Fresh research outputs after research GPU0 OOM.'}
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True); OUT_JSON.write_text(json.dumps(payload, indent=2, ensure_ascii=False)+'\n', encoding='utf-8')
    lines = ['# research same-entity pair discriminator 1M profile','',f'Evidence JSON: `{OUT_JSON}`','',
    ...
    for a,d in rows.items():
        m=d['metrics_summary']
        lines.append(f"| {a} | {m.get('loss_first')} | {m.get('loss_last')} | {m.get('steps')} | {m.get('truncated_fraction')} | {m.get('tokens_per_word_kept')} | {m.get('masked_tokens_per_word')} |")
    OUT_NOTE.parent.mkdir(parents=True, exist_ok=True); OUT_NOTE.write_text('\n'.join(lines)+'\n', encoding='utf-8')
    print(json.dumps({'out':str(OUT_JSON),'note':str(OUT_NOTE),'true_minus_hardneg':primary,'elapsed_sec':payload['elapsed_sec']}, indent=2))
