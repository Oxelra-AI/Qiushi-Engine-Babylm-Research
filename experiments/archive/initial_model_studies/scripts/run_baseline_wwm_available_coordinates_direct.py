...
BACKEND = "mlm"
RUNNER_NOTE = "Direct local checkpoint evaluation for protected ordinary WWM baseline: model_path_or_name is hf_model/chck_*M; no local revision selection is used."
OUT_MANIFEST = ROOT / "data/baseline_wwm_available_coordinate_manifest.json"
NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/baseline_wwm_available_coordinate_manifest.md')
...
JOBS = [
    ("baseline_wwm_10M", ROOT / "training/runs/babylm_fullcycle_debertav2_8x480_wwm_seed42_100M_b256/hf_model/chck_10M"),
    ("baseline_wwm_20M", ROOT / "training/runs/babylm_fullcycle_debertav2_8x480_wwm_seed42_100M_b256/hf_model/chck_20M"),
]
...
def eval_job(key: str, model_path: pathlib.Path) -> dict:
    ...
    env = setup_env()
    outdir = ROOT / f"training/runs/baseline_wwm_eval_{key}"
    outdir.mkdir(parents=True, exist_ok=True)
    log_path = ROOT / f"notes/baseline_wwm_eval_{key}.log"
    ...
    out_json = ROOT / f"data/baseline_wwm_{key}_available_coordinate.json"
    out_note = ROOT / f"notes/107_baseline_wwm_{key}_available_coordinate.md"
    ...
    lines = [f"# research baseline WWM available coordinate — {key}", "", f"Evidence JSON: `{out_json}`", "", "| column/task | score |", "|---|---:|"]
...
def main() -> None:
    ...
    lines = ["# research — baseline WWM available-coordinate manifest", "", f"Evidence JSON: `{OUT_MANIFEST}`", "", RUNNER_NOTE, ""]
    for r in results:
    ...
    NOTE.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"status": "BASELINE_WWM_AVAILABLE_COORDINATES_DONE", "manifest": str(OUT_MANIFEST), "elapsed_sec": time.time() - t0}, indent=2))
