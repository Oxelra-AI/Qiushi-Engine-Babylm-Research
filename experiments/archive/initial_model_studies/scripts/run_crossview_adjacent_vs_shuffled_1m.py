...
OUT_JSON = ROOT / "data/crossview_adjacent_vs_shuffled_1m_profile.json"
OUT_TRAIN_JSON = ROOT / "data/crossview_adjacent_vs_shuffled_1m_training_summary.json"
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/crossview_adjacent_vs_shuffled_1m_profile.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/crossview_adjacent_vs_shuffled_1m_train_and_profile.log')
...
def run_id_for(arm: str, seed: int) -> str:
    return f"babylm_step65_crossview_{arm}_wwm_seed{seed}_1M"
...
def compare_supervision(adj_manifest: list[dict], shuf_manifest: list[dict]) -> dict:
    """Check the intended cross-view supervision invariant.

    Official rows are identical across adjacent/shuffled arms and should match in
    ordinary text-derived counts. Pair rows deliberately have different visible
    target text, so their total words/kept tokens may differ. For pair rows the
    scientific invariant is that the *predicted source-side anchors* are copied
    from the true pair unchanged: same source pair, same anchor indices/norms,
    same anchor groups, and same anchor token count. Target-pair IDs, total
    words, and total kept tokens are allowed to differ for pair rows.
    """
    if len(adj_manifest) != len(shuf_manifest):
        return {"ok": False, "reason": "length mismatch", "len_adj": len(adj_manifest), "len_shuf": len(shuf_manifest)}
    mismatches = []
    pair_count = 0
    official_count = 0
    for a, s in zip(adj_manifest, shuf_manifest):
        if a.get("row_index") != s.get("row_index") or a.get("example_id") != s.get("example_id") or a.get("kind") != s.get("kind"):
            mismatches.append({"row_index": a.get("row_index"), "bad": {"row/example/kind": ((a.get("row_index"), a.get("example_id"), a.get("kind")), (s.get("row_index"), s.get("example_id"), s.get("kind")))}})
            continue
        if a["kind"] == "pair_crossview":
            pair_count += 1
            fields = ["source_pair_id", "anchor_group_count", "anchor_token_count", "anchor_source_word_indices", "anchor_norms"]
            ...
            if bad:
                mismatches.append({"row_index": a.get("row_index"), "bad": bad})
    return {"ok": len(mismatches) == 0, "pair_count": pair_count, "official_count": official_count, "mismatches_first10": mismatches[:10]}


def train_and_check(seed: int, arm: str, seed_meta: dict, env: dict[str, str], logf) -> dict:
    run_id = run_id_for(arm, seed)
    run_dir = RUN_ROOT / run_id
    jsonl = seed_meta[f"full_{arm}_path"]
    cmd = [
        sys.executable, str(TRAIN),
        "--output_dir", str(run_dir),
        "--example_jsonl", jsonl,
        "--example_jsonl_label", f"crossview_{arm}_1m",
...
def profile(run_id: str, env: dict[str, str], logf) -> dict:
    ...
    outdir = (run_dir / "eval_results_revision_65").resolve()
    scores, reports = {}, {}
...
def main() -> None:
    ...
    with LOG.open("a", encoding="utf-8") as logf:
        for seed_str, seed_meta in meta["seeds"].items():
            seed = int(seed_str)
            assert seed_meta["full_adjacent_summary"]["total_words"] == 1_000_000
            assert seed_meta["full_shuffled_summary"]["total_words"] == 1_000_000
            assert seed_meta["per_pair_supervision_proxy_identical_full"] is True
            ...
            cmp = compare_supervision(train_rows[adj_id]["supervision_manifest"], train_rows[shuf_id]["supervision_manifest"])
            if not cmp["ok"]:
    ...
    for seed_str in meta["seeds"]:
        seed = int(seed_str)
        adj_id = run_id_for("adjacent", seed); shuf_id = run_id_for("shuffled", seed)
        adj = profile_rows[adj_id]["scores"]; shuf = profile_rows[shuf_id]["scores"]
        per_seed_scores[seed_str] = {"crossview_adjacent": adj, "crossview_shuffled": shuf}
        deltas[seed_str] = {c: round(adj[c] - shuf[c], 4) for c in COLS}
    ...
    payload = {
        "crossview_materialization_meta": str(META),
        "per_seed_supervision_identity": sup_cmp,
        "train_summary": train_summary,
        "per_seed_scores": per_seed_scores,
        "crossview_adjacent_minus_crossview_shuffled_by_seed": deltas,
        "mean_crossview_adjacent_minus_crossview_shuffled": mean_delta,
    }
    ...
    lines = [
        "# research — cross-view masking: adjacent vs shuffled",
        "",
    ...
    for seed_str in meta["seeds"]:
        for key in ["crossview_adjacent", "crossview_shuffled"]:
    ...
    lines += ["", "## adjacent minus shuffled", "", "| seed | BLiMP | Supp | EWoK | Entity | COMPS | Read eye | Read SPR |", "|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for seed_str in meta["seeds"]:
        d = deltas[seed_str]
    ...
    OUT_NOTE.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"mean_crossview_adjacent_minus_crossview_shuffled": mean_delta, "by_seed": deltas, "supervision_identity": sup_cmp, "out": str(OUT_JSON)}, indent=2, ensure_ascii=False))
