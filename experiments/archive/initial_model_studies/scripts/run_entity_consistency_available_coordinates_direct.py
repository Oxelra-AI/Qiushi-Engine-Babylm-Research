...
OUT_MANIFEST = ROOT / "data/entity_consistency_available_coordinate_manifest.json"
NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/entity_consistency_available_coordinate_manifest.md')
...
JOBS = [
    ("consistency_10M", ROOT / "training/runs/babylm_entity_consistency_deberta_20M/hf_model/chck_10M"),
    ("consistency_20M", ROOT / "training/runs/babylm_entity_consistency_deberta_20M/hf_model/chck_20M"),
    ("shuffled_10M", ROOT / "training/runs/babylm_entity_shuffled_pair_deberta_20M/hf_model/chck_10M"),
    ("shuffled_20M", ROOT / "training/runs/babylm_entity_shuffled_pair_deberta_20M/hf_model/chck_20M"),
]
...
def eval_job(key: str, model_path: pathlib.Path) -> dict:
    ...
    env = setup_env()
    outdir = ROOT / f"training/runs/entity_consistency_eval_{key}"
    outdir.mkdir(parents=True, exist_ok=True)
    log_path = ROOT / f"notes/entity_consistency_eval_{key}.log"
    ...
    out_json = ROOT / f"data/entity_consistency_{key}_available_coordinate.json"
    out_note = ROOT / f"notes/103_entity_consistency_{key}_available_coordinate.md"
    ...
    lines = [f"# research available coordinate — {key}", "", f"Evidence JSON: `{out_json}`", "", "| column/task | score |", "|---|---:|"]
...
def main() -> None:
    ...
    lines = ["# research — Entity consistency available-coordinate manifest", "", f"Evidence JSON: `{OUT_MANIFEST}`", "", RUNNER_NOTE, ""]
    for r in results:
    ...
    NOTE.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"status": "ENTITY_AVAILABLE_COORDINATES_DONE", "manifest": str(OUT_MANIFEST), "elapsed_sec": time.time() - t0}, indent=2))
