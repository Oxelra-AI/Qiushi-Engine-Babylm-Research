...
OUT_JSON = ROOT / "data/masked_1m_grid_pos512_profile.json"
OUT_TRAIN_JSON = ROOT / "data/masked_1m_grid_pos512_training_summary.json"
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/masked_1m_pos512_token_vs_wwm_profile.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/masked_1m_pos512_train_and_profile.log')

RUN_SPECS = [("token", "babylm_masked_token_pos512_1M"), ("wwm", "babylm_masked_wwm_pos512_1M")]
TASKS = [
...
def train_and_check(mask_mode: str, run_id: str, env: dict[str, str], logf) -> dict:
    ...
    for rel in ["hf_model", "hf_model/chck_1M"]:
        ...
        assert int(model.config.max_position_embeddings) >= 512, rel
    ...
    assert metrics["max_seq_length"] == 256
...
def profile(run_id: str, env: dict[str, str], logf) -> dict:
    ...
    outdir = (run_dir / "eval_results_masked_1m_pos512").resolve()
    scores = {}
    reports = {}
...
def main() -> None:
    ...
    for r in train_rows:
        ...
        train_summary.append({
            ...
            "source_words": r["manifest"]["source_words_consumed"],
            "first12_examples": r["manifest"]["consumed_example_ids_in_order"][:12],
            "load_checks": r["load_checks"],
        })
    ...
    row_by_mode = {r["run_id"].split("masked_")[1].split("_pos512_1M")[0]: r for r in profile_rows}
    ...
    lines = [
        "# research — 1M masked-LM token masking vs whole-word masking, position-capacity repaired",
        "",
