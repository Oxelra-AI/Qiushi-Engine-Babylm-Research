...
def main() -> None:
    ...
    metrics = {}
    for mode in MODES:
        run_id = f"babylm_step30_order_{mode}_pool900k_train10k_smoke"
        run_training([
        ...
        root, ckpt = d["loads"]
        print("SMOKE", mode, "root", root["ok"], "ckpt", ckpt["ok"], "revision", d["revision_load"]["ok"], "params", m["parameter_count"], "order", m.get("order_mode"), "source_words", e["source_words_consumed"], "first", e["consumed_example_ids_in_order"][:10])
        assert root["ok"] and ckpt["ok"] and d["revision_load"]["ok"]
        assert m.get("order_mode") == mode
        assert e.get("order_mode") == mode
        assert m.get("example_pool_words_actual") == 900000
    ...
    assert len(source_words["random"]) >= 2, source_words["random"]
    ...
    summary = {
        ...
        "source_stage_order_differs": True,
        "readability_order_differs": True,
        "source_words": source_words["random"],
        "first20": {mode: final_order[mode][:20] for mode in MODES},
        "loss_last": {mode: metrics[mode]["loss_last"] for mode in MODES},
        "order_info": {mode: manifests[mode].get("order_info") for mode in MODES},
    }
    out = ROOT / "data/order_smoke_multisource_summary.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    print("ORDER_MULTISOURCE_SMOKE_OK", json.dumps(summary, indent=2, ensure_ascii=False))
