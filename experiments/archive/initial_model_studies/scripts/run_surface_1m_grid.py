...
def main() -> None:
    ...
    with LOG.open("a", encoding="utf-8") as logf:
        logf.write(f"\n=== research surface 1M grid start {datetime.utcnow().isoformat()}Z ===\n")
        for seed in SEEDS:
            baseline_run = f"babylm_step26_ctrl_dense_pool10M_seed{seed}_1M"
            baseline_manifest = load_json(RUN_ROOT / baseline_run / "example_order_manifest.json")
            baseline_order = baseline_manifest["consumed_example_ids_in_order"]
            # research random baselines predate `selected_example_ids_before_order`; for random order, the selected order is the consumed order.
            ...
            for variant, short in SPECS:
                run_id = f"babylm_step36_{short}_pool10M_seed{seed}_1M"
                cmd = [
                ...
                print("TRAINED_SURFACE", run_id, "params", metrics["parameter_count"], "loss", round(metrics["loss_first"], 4), "->", round(metrics["loss_last"], 4), "surface", metrics.get("surface_summary_last"))
