...
OUTDIR = (ROOT / "training/runs/babylm_fullcycle_wwm_seed42_100M/eval_results_aoa").resolve()
OUT_JSON = ROOT / "data/wwm100m_aoa_result.json"
OUT_NOTE = (ROOT.parents[2] / 'research/notes/initial_model_studies/wwm100m_aoa_result.md')
LOG = (ROOT.parents[2] / 'research/notes/initial_model_studies/wwm100m_aoa_eval.log')
...
def main():
    ...
    surprisal_path = OUTDIR / SURPRISAL_REL
    if not score_path.exists():
        # Fallback: locate score file if model path stem/layout differs.
    ...
    payload = {"model_path": str(MODEL), "backend": "mlm", "track_name": "strict-small", "word_path": str(WORD_PATH), "output_dir": str(OUTDIR), "score_path": str(score_path), "surprisal_path": str(surprisal_path if surprisal_path.exists() else score_path.parent / 'surprisal.json'), "aoa": aoa}
    OUT_JSON.parent.mkdir(parents=True, exist_ok=True)
    OUT_JSON.write_text(json.dumps(payload, indent=2) + "\n")
    OUT_NOTE.write_text(f"# research WWM100M AoA\n\nAoA: {aoa:.4f}\n\nEvidence: `{OUT_JSON}`\nRaw score: `{score_path}`\n", encoding="utf-8")
