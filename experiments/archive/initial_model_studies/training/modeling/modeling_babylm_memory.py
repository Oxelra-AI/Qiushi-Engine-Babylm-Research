...
class PrefixMemoryAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor | None = None) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        if self.memory_enabled:
            ...
            # Non-persistent capacity control: keep the same projections/gates but
            # remove cross-token prefix accumulation, so any effect reflects extra
            # per-token parameters/optimization rather than state persistence.
            mem = gate * value
        read = self.read(mem)
        ...
        stats = {
            ...
            "memory_enabled": torch.tensor(float(self.memory_enabled), device=x.device),
        }
        return y, stats
