...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--batch_size", type=int, default=64, help="Microbatch size when --grad_accum_steps > 1")
    p.add_argument("--grad_accum_steps", type=int, default=1, help="Accumulate this many microbatches per optimizer step; effective batch = batch_size * grad_accum_steps except possibly final step")
...
def main() -> None:
    ...
    if args.grad_accum_steps < 1:
        raise RuntimeError(f"grad_accum_steps must be >= 1, got {args.grad_accum_steps}")
    ...
    if schedule_total < total_steps:
        raise RuntimeError(f"lr_total_steps {schedule_total} < actual optimizer steps {total_steps} (micro_steps={total_micro_steps}, grad_accum_steps={args.grad_accum_steps})")
    ...
    accum_seq_lens: list[int] = []
    optimizer_step = 0
    optim.zero_grad(set_to_none=True)
    with log_path.open("w", encoding="utf-8") as logf:
        for micro_step, batch in enumerate(loader, 1):
            group_start_micro = ((micro_step - 1) // args.grad_accum_steps) * args.grad_accum_steps + 1
            actual_accum = min(args.grad_accum_steps, total_micro_steps - group_start_micro + 1)
            pending_optimizer_step = ((micro_step - 1) // args.grad_accum_steps) + 1
            words = int(batch.pop("words").sum().item())
            input_ids = batch["input_ids"].to(device, non_blocking=True)
            attention_mask = batch["attention_mask"].to(device, non_blocking=True)
            word_group = batch["word_group"].to(device, non_blocking=True)
            frac = (pending_optimizer_step - 1) / max(1, schedule_total)
            ...
            loss_float_micro = float(loss.detach().cpu())
            n_pred = int((labels != -100).sum().item())
            accum_words += words
            accum_loss_sum += loss_float_micro
            accum_masked_tokens += n_pred
            accum_seq_lens.append(cur_len)
            is_update = (micro_step % args.grad_accum_steps == 0) or (micro_step == total_micro_steps)
            if not is_update:
                continue
            optimizer_step += 1
            ...
            sched.step()
            optim.zero_grad(set_to_none=True)
            cumulative_words += accum_words
            loss_float = accum_loss_sum / len(accum_seq_lens)
            loss_values.append(loss_float)
            masked_token_values.append(accum_masked_tokens)
            seq_len_values.extend(accum_seq_lens)
            rec = {"step": optimizer_step, "micro_step": micro_step, "loss": loss_float, "lr": float(sched.get_last_lr()[0]),
                   "batch_words": accum_words, "microbatch_size": args.batch_size, "grad_accum_steps": args.grad_accum_steps,
                   "cumulative_word_exposure": cumulative_words, "seq_len": max(accum_seq_lens),
                   "masked_tokens": accum_masked_tokens, "elapsed_sec": time.time() - start_time}
