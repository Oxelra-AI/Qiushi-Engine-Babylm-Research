...
def make_mntp_labels(wwm_labels: torch.Tensor) -> torch.Tensor:
    """Shift masked labels left so output at k predicts masked token k+1."""
    labels = torch.full_like(wwm_labels, -100)
    labels[:, :-1] = wwm_labels[:, 1:]
    return labels


def make_causal_attention_mask(attention_mask_2d: torch.Tensor) -> torch.Tensor:
    """Return [batch, query, key] lower-triangular mask with padding removed."""
    if attention_mask_2d.dim() != 2:
        raise ValueError(f"expected 2D attention mask, got {tuple(attention_mask_2d.shape)}")
    bsz, seq = attention_mask_2d.shape
    causal = torch.tril(torch.ones((seq, seq), dtype=attention_mask_2d.dtype,
                                   device=attention_mask_2d.device))
    return (causal.unsqueeze(0).expand(bsz, -1, -1)
            * attention_mask_2d.unsqueeze(-1) * attention_mask_2d.unsqueeze(1))


def objective_for_step(step: int, mntp_every: int) -> str:
    """Deterministic 1:(N-1) assignment: every Nth optimizer batch is MNTP."""
    return "mntp" if mntp_every > 0 and step % mntp_every == 0 else "wwm"
...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--mntp_every", type=int, default=0, help="Every Nth batch is MNTP; use 16 for deterministic 1:15 MNTP:WWM")
...
def main() -> None:
    ...
    (out / "example_order_manifest.json").write_text(json.dumps({
        ...
        "mntp_every": args.mntp_every,
        "objective_assignment": (f"deterministic_every_{args.mntp_every}" if args.mntp_every > 0 else "wwm_only"),
    ...
    objective_step_counts: dict[str, int] = {"wwm": 0, "mntp": 0}
    objective_word_counts: dict[str, int] = {"wwm": 0, "mntp": 0}
    objective_predicted_token_counts: dict[str, int] = {"wwm": 0, "mntp": 0}
    ...
    with log_path.open("w", encoding="utf-8") as logf:
        for step, batch in enumerate(loader, 1):
            ...
            # Consume the exact ordinary S1 WWM draw on every batch, preserving
            # baseline corruption positions and RNG trajectory.
            masked_inputs, wwm_labels = apply_masking(input_ids, attention_mask, word_group, tokenizer, cur_mask_mode, args.mask_prob, gen)
            objective = objective_for_step(step, args.mntp_every)
            if objective == "mntp":
                labels = make_mntp_labels(wwm_labels)
                causal_4d = make_causal_attention_mask(attention_mask).unsqueeze(1)  # [B,1,S,S]
            else:
                labels = wwm_labels
                causal_4d = None
            n_pred = int((labels != -100).sum().item())
            optim.zero_grad(set_to_none=True)
            # On MNTP batches, monkey-patch encoder.get_attention_mask to return
            # the causal mask while the model still receives the 2D mask for its
            # embedding layer.  WWM batches use the normal path unchanged.
            _orig_get_mask = model.deberta.encoder.get_attention_mask
            if causal_4d is not None:
                model.deberta.encoder.get_attention_mask = lambda _mask, _c=causal_4d: _c
            if micro_batch_size and 0 < micro_batch_size < masked_inputs.shape[0]:
                ...
                for lo in range(0, masked_inputs.shape[0], micro_batch_size):
                    ...
                    if causal_4d is not None:
                        model.deberta.encoder.get_attention_mask = lambda _mask, _c=causal_4d[lo:hi]: _c
            ...
            model.deberta.encoder.get_attention_mask = _orig_get_mask
            ...
            objective_step_counts[objective] += 1
            objective_word_counts[objective] += words
            objective_predicted_token_counts[objective] += n_pred
    ...
    metrics = {
        "variant": (f"hybrid_mntp_every_{args.mntp_every}" if args.mntp_every > 0 else (f"masked_{args.mask_mode}" if not args.mask_mode_schedule else "masked_curriculum")),
        "backend": "mlm",
        ...
        "objective_word_counts": objective_word_counts,
        "objective_predicted_token_counts": objective_predicted_token_counts,
