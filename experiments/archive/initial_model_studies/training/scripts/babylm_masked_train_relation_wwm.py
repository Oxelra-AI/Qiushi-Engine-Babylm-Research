...
# Lightweight deterministic lexicons for relation/entity-biased WWM. These are
# not learned from extra text and therefore do not add language-data exposure.
PRONOUN_WORDS = {
    "he", "she", "they", "them", "him", "her", "his", "hers", "their", "theirs", "it", "its", "we", "us", "i", "you"
}
RELATION_WORDS = {
    "in", "on", "under", "over", "above", "below", "behind", "beside", "near", "far", "inside", "outside",
    "left", "right", "front", "back", "before", "after", "between", "through", "into", "onto", "from", "to",
    "with", "without", "around", "across", "next", "during", "while", "because", "if", "then", "than"
}
STATE_EVENT_WORDS = {
    "put", "puts", "putting", "move", "moves", "moved", "moving", "take", "takes", "took", "taken", "taking",
    "give", "gives", "gave", "given", "bring", "brings", "brought", "leave", "leaves", "left", "enter", "enters",
    "entered", "exit", "exits", "open", "opens", "opened", "close", "closes", "closed", "contain", "contains",
    "hold", "holds", "held", "break", "breaks", "broke", "broken", "fall", "falls", "fell", "spill", "spills",
    "fill", "fills", "filled", "empty", "empties", "drop", "drops", "dropped", "pick", "picked", "push", "pull"
}
PROPERTY_AFFORDANCE_WORDS = {
    "hard", "soft", "heavy", "light", "sharp", "dull", "wet", "dry", "hot", "cold", "warm", "cool", "glass",
    "metal", "paper", "wood", "wooden", "rubber", "cloth", "water", "fire", "heat", "transparent", "clear",
    "bend", "bends", "cut", "cuts", "float", "floats", "sink", "sinks", "fit", "fits", "bigger", "smaller",
    "longer", "shorter", "full", "empty", "round", "flat", "open", "closed"
}
COMMON_ENTITY_WORDS = {
    "person", "man", "woman", "boy", "girl", "child", "friend", "mother", "father", "sister", "brother", "teacher",
    "dog", "cat", "bird", "animal", "box", "bag", "cup", "bottle", "bucket", "jar", "basket", "room", "house", "door",
    "window", "table", "chair", "shelf", "drawer", "book", "paper", "ball", "coin", "key", "toy", "car", "knife",
    "rope", "towel", "glass", "water", "apple", "food", "shoe", "hat", "coat"
}
STOPWORD_LIKE = {
    "the", "a", "an", "and", "or", "but", "of", "for", "is", "are", "was", "were", "be", "been", "being",
    "this", "that", "these", "those", "there", "here", "not", "very", "just", "as", "at", "by"
}


def normalize_token_piece(piece: str) -> str:
    piece = str(piece).replace("Ġ", "").replace("▁", "")
    piece = piece.replace("Ċ", " ")
    piece = re.sub(r"[^A-Za-z0-9'-]+", "", piece)
    return piece.lower()


def group_texts_for_example(input_ids_b: torch.Tensor, groups_b: torch.Tensor, tokenizer) -> dict[int, str]:
    pieces: dict[int, list[str]] = {}
    for pos in range(int(groups_b.shape[0])):
        g = int(groups_b[pos].item())
        if g < 0:
            continue
        tok = tokenizer.convert_ids_to_tokens(int(input_ids_b[pos].item()))
        pieces.setdefault(g, []).append(normalize_token_piece(tok))
    out: dict[int, str] = {}
    for g, ps in pieces.items():
        txt = "".join(ps).strip("-'")
        if txt:
            out[g] = txt
    return out
...
def relation_group_features(texts: dict[int, str]) -> dict[int, dict[str, int]]:
    counts = Counter(t for t in texts.values() if t)
    feats: dict[int, dict[str, int]] = {}
    for g, txt in texts.items():
        alpha = txt.isalpha()
        repeated_content = alpha and len(txt) >= 4 and txt not in STOPWORD_LIKE and counts[txt] > 1
        entity = int(txt in COMMON_ENTITY_WORDS or txt in PRONOUN_WORDS or repeated_content or (alpha and len(txt) >= 7 and counts[txt] > 1))
        relation = int(txt in RELATION_WORDS or txt in STATE_EVENT_WORDS)
        state = int(txt in PROPERTY_AFFORDANCE_WORDS)
        feats[g] = {"entity": entity, "relation": relation, "state": state, "any": int(entity or relation or state)}
    return feats
...
def apply_masking(input_ids: torch.Tensor, attention_mask: torch.Tensor, word_group: torch.Tensor,
                  ...
                  relation_entity_alpha: float = 2.0, relation_word_beta: float = 2.0,
                  relation_state_gamma: float = 2.0) -> tuple[torch.Tensor, torch.Tensor, dict]:
    """Return (masked_input_ids, labels, telemetry). labels=-100 where not predicted.

    `wwm` preserves the trusted independent-Bernoulli WWM route. `wwm_exact`,
    `relation_wwm`, and `relation_wwm_shuffled` select exactly K whole-word groups
    per example, K = clip(round(mask_prob * num_valid_groups), 1, n). Relation
    modes change only the group sampling weights; selected-token loss, 80/10/10
    replacement, data order, exposure accounting, checkpoints, and model stay fixed.
    """
    ...
    tele = {"examples": float(bsz), "candidate_groups": 0.0, "selected_groups": 0.0,
            "relation_candidate_groups": 0.0, "relation_selected_groups": 0.0,
            "entity_candidate_groups": 0.0, "entity_selected_groups": 0.0,
            "state_candidate_groups": 0.0, "state_selected_groups": 0.0}
    if mask_mode == "token":
        ...
        for b in range(bsz):
            ...
            if valid_groups.numel() == 0:
                continue
            tele["candidate_groups"] += float(valid_groups.numel())
            gp = torch.rand(valid_groups.numel(), generator=gen, device=device)
            chosen = valid_groups[gp < mask_prob]
            if chosen.numel() == 0:
                continue
            tele["selected_groups"] += float(chosen.numel())
            sel_b = torch.isin(groups, chosen) & candidate[b]
            select[b] = sel_b
    elif mask_mode in {"wwm_exact", "relation_wwm", "relation_wwm_shuffled"}:
        for b in range(bsz):
            groups = word_group[b]
            valid_groups = torch.unique(groups[groups >= 0]).sort().values
            n = int(valid_groups.numel())
            if n == 0:
                continue
            k = max(1, min(n, int(round(mask_prob * n))))
            weights = torch.ones(n, dtype=torch.float32, device=device)
            feat_by_group: dict = {}
            if mask_mode.startswith("relation_wwm"):
                texts = group_texts_for_example(input_ids[b].detach().cpu(), groups.detach().cpu(), tokenizer)
                feat_by_group = relation_group_features(texts)
                gl = valid_groups.detach().cpu().tolist()
                for i, g in enumerate(gl):
                    f = feat_by_group.get(int(g), {"entity": 0, "relation": 0, "state": 0, "any": 0})
                    weights[i] += relation_entity_alpha * float(f["entity"])
                    weights[i] += relation_word_beta * float(f["relation"])
                    weights[i] += relation_state_gamma * float(f["state"])
                if mask_mode == "relation_wwm_shuffled" and n > 1:
                    perm = torch.randperm(n, generator=gen, device=device)
                    weights = weights[perm]
            probs = weights / weights.sum()
            chosen_idx = torch.multinomial(probs, num_samples=k, replacement=False, generator=gen)
            chosen = valid_groups[chosen_idx]
            tele["candidate_groups"] += float(n)
            tele["selected_groups"] += float(k)
            if feat_by_group:
                chosen_set = set(int(x) for x in chosen.detach().cpu().tolist())
                for g in valid_groups.detach().cpu().tolist():
                    f = feat_by_group.get(int(g), {"entity": 0, "relation": 0, "state": 0, "any": 0})
                    tele["relation_candidate_groups"] += float(f["relation"])
                    tele["entity_candidate_groups"] += float(f["entity"])
                    tele["state_candidate_groups"] += float(f["state"])
                    if int(g) in chosen_set:
                        tele["relation_selected_groups"] += float(f["relation"])
                        tele["entity_selected_groups"] += float(f["entity"])
                        tele["state_selected_groups"] += float(f["state"])
            sel_b = torch.isin(groups, chosen) & candidate[b]
            select[b] = sel_b
    else:
    ...
    tele["selected_tokens"] = float((labels != -100).sum().item())
...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--mask_mode", choices=["token", "wwm", "wwm_exact", "relation_wwm", "relation_wwm_shuffled"], default="token")
    p.add_argument("--relation_entity_alpha", type=float, default=2.0, help="Extra weight for entity/object/pronoun/repeated-content groups")
    p.add_argument("--relation_word_beta", type=float, default=2.0, help="Extra weight for spatial/temporal/transfer relation groups")
    p.add_argument("--relation_state_gamma", type=float, default=2.0, help="Extra weight for state/property/material/affordance groups")
...
def main() -> None:
    ...
    with log_path.open("w", encoding="utf-8") as logf:
        for step, batch in enumerate(loader, 1):
            ...
            masked_inputs, labels, mask_telemetry = apply_masking(
                input_ids, attention_mask, word_group, tokenizer, args.mask_mode, args.mask_prob, gen,
                relation_entity_alpha=args.relation_entity_alpha,
                relation_word_beta=args.relation_word_beta,
                relation_state_gamma=args.relation_state_gamma,
            )
            ...
            for _mk, _mv in mask_telemetry.items():
                rec[f"mask_{_mk}"] = _mv
                masking_telemetry_totals[_mk] = masking_telemetry_totals.get(_mk, 0.0) + float(_mv)
    ...
    masking_telemetry_mean_per_step = {k: v / max(1, total_steps) for k, v in masking_telemetry_totals.items()}
    masking_telemetry_summary = {
        "mask_mode": args.mask_mode,
        "relation_entity_alpha": args.relation_entity_alpha,
        "relation_word_beta": args.relation_word_beta,
        "relation_state_gamma": args.relation_state_gamma,
        "totals": masking_telemetry_totals,
        "mean_per_step": masking_telemetry_mean_per_step,
    }
    (out / "masking_telemetry_summary.json").write_text(json.dumps(masking_telemetry_summary, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    metrics = {
        ...
        "relation_word_beta": args.relation_word_beta,
        "relation_state_gamma": args.relation_state_gamma,
        "masking_telemetry_summary": masking_telemetry_summary,
