#!/usr/bin/env python3
"""research: train HALF_VIEW no-exact-copy DeBERTa control.

The stream is materialized by `materialize_half_view_control.py`.
This wrapper holds the established DeBERTa MLM recipe fixed and changes only the
example stream. It is a boundary/composition test: half local VIEW dose without
exact-recurrence companion competition.
"""
...
ROOT = _public_path('experiments/archive/relation_learning/scripts/train_half_view_control.py')
ROOT = _PUBLIC_ROOT
...
META_PATH = _public_path('experiments/archive/relation_learning/data/half_view_control_pools/half_view_control_metadata.json')
STREAM_KEY = "compact_half_view_noexact_dose2p64x_100M"
STREAM_SHA_KEY = "compact_half_view_noexact_dose2p64x_100M.jsonl"
STREAM_LABEL = "compact_half_view_noexact_dose2p64x_matched_rowholdout"
PREFLIGHT_DIR = _public_path('experiments/archive/relation_learning/data/half_view_train_preflight')
...
def default_run_dir(seed: int) -> pathlib.Path:
    return _public_path('experiments/archive/relation_learning/training/runs') / f"full_p2c_c2p_abs_half_view_noexact_dose2p64x_matched_rowholdout_deberta100M_seed{seed}"
...
def preflight(run_dir: pathlib.Path, count_words: bool) -> dict[str, Any]:
    ...
    if meta.get("status") != "HALF_VIEW_CONTROL_MATERIALIZED":
        raise RuntimeError(f"unexpected HALF_VIEW metadata status {meta.get('status')}")
    ...
    for k, v in required.items():
        if audit.get(k) != v:
            raise RuntimeError(f"HALF_VIEW metadata invariant failed for {k}: {audit.get(k)}")
    if not audit.get("pool", {}).get("exact_10M"):
        raise RuntimeError(f"HALF_VIEW pool not exact: {audit.get('pool')}")
    if int(audit.get("view_relation_pairs") or 0) != 16731 or int(audit.get("neutral_no_exact_pairs") or 0) != 16560:
        raise RuntimeError(f"HALF_VIEW assignment counts unexpected: {audit}")
    ...
    return {
        "status": "HALF_VIEW_TRAIN_PREFLIGHT_OK",
        "created_utc": now(),
        "run_dir": rel(run_dir),
        "stream": rel(stream),
        "stream_sha256": stream_sha,
        "stream_accounting": counts,
        "half_view_metadata": rel(META_PATH),
        ...
        "scientific_purpose": "Half-dose VIEW relation without exact recurrence: distinguishes HM dose curve from active exact/restatement competition.",
        "pre_state_numeric_reading": rel(_public_path('research/notes/relation_learning/numerical_repair_and_halfview_prestate.md')),
...
def main() -> None:
    ...
    write_json(PREFLIGHT_DIR / f"half_view_seed{args.seed_replicate}_preflight.json", info)
    if args.dry_run:
        print(json.dumps({
            "status": info["status"],
            ...
            "preflight_json": rel(PREFLIGHT_DIR / f"half_view_seed{args.seed_replicate}_preflight.json"),
            "no_training_started": True,
        }, indent=2, ensure_ascii=False), flush=True)
        return
    existing = summarize_metrics(run_dir)
    if existing.get("ready"):
        print(json.dumps({"status": "HALF_VIEW_ALREADY_FINISHED", "run_dir": rel(run_dir), "metrics": existing}, indent=2, ensure_ascii=False), flush=True)
        return
    ...
    write_json(run_dir / "half_view_train_preflight.json", info)
    env = os.environ.copy()
    env["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    env["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"
    env.setdefault("TOKENIZERS_PARALLELISM", "false")
    env.setdefault("PYTHONDONTWRITEBYTECODE", "1")
    env["TMPDIR"] = f"/tmp/q_relation_learning_step026_half_view_seed{args.seed_replicate}_{int(time.time())}"
    ...
    start = now()
    print(json.dumps({
        "status": "HALF_VIEW_TRAIN_STARTING",
        "seed": args.seed_replicate,
        "gpu": args.gpu,
        "run_dir": rel(run_dir),
        "created_utc": start,
    ...
    with stdout_path.open("w", encoding="utf-8") as out, stderr_path.open("w", encoding="utf-8") as err:
        out.write(json.dumps({"event": "half_view_train_start", "utc": start, "seed": args.seed_replicate, "gpu": args.gpu, "recipe": base.RECIPE}, ensure_ascii=False) + "\n")
        out.flush()
    ...
    result = {
        "status": "HALF_VIEW_TRAIN_FINISHED" if proc.returncode == 0 and metrics.get("ready") else "HALF_VIEW_TRAIN_INCOMPLETE",
    ...
    write_json(run_dir / "half_view_train_result.json", result)
    print(json.dumps(result, indent=2, ensure_ascii=False), flush=True)
    if result["status"] != "HALF_VIEW_TRAIN_FINISHED":
