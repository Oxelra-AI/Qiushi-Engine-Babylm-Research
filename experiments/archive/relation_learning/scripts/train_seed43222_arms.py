#!/usr/bin/env python3
"""research: train third-seed DeBERTa MAX-dose arms for the repetition/variation trade-off.

Scientific role
---------------
relation_learning Steps 5-6 found that aggregate fixed-budget substitution is not stable,
but Entity splits into a strong two-sided operation-depth dissociation: exact
repetition buys shallow 0-operation recall, while distinct renderings buy deeper
state updating. A third seed is now the minimum useful replication of that
specific dissociation.

This launcher trains only mechanism-instrument DeBERTa arms under the already
materialized frontier_consolidation MAX-dose streams and tokenizer:
  * view   = compact distinct renderings at 2.64x dose
  * repeat = exact repetition of the matched compact content at 2.64x dose
  * clean  = length/row/update matched clean-Qwen control

All outputs for seed43222 are written into relation_learning, not frontier_consolidation. No evaluation,
SuperGLUE, AoA, upload, or leaderboard action is performed here.
"""
...
ROOT = find_root()
FUNCTIONAL_RELATION_STUDIES_WS = ROOT / "experiments/archive" / 'relation_learning'
...
POOL_DIR = REPRESENTATION_FRONTIER_STUDIES_WS / "data" / "dose_2p64x_rowholdout_pools"
META_PATH = POOL_DIR / "dose2p64x_rowholdout_metadata.json"
CLEAN_MAT_PATH = POOL_DIR / "cleanqwen_lengthmatched_dose2p64x_100M.materialization.json"
CLEAN_STREAM = POOL_DIR / "cleanqwen_lengthmatched_dose2p64x_100M.jsonl"
RUNS_DIR = FUNCTIONAL_RELATION_STUDIES_WS / "training" / "runs"

EXPECTED_TOKENIZER_SHA = "91b775514b9f4e2d1f37c28445ab98181007e16d14f763955168547e293ee8f9"
EXPECTED_CLEAN_STREAM_SHA = "64e686d16e0d7d5e81acecc73494d8670a1d6f8ddaffb4ce517277c983a9bed8"
...
RECIPE = {
    ...
    "ffn_mult": 4,
    "seed": 43,
    "extra_init_seed": 43222,
    "train_rng_seed": 43223,
    "batch_size": 256,
    "seq_length": 256,
    "max_seq_length": 256,
    "learning_rate": 0.001,
...
STREAM_LABELS = {
    "view": "seed43222_compact_view_dose2p64x_matched_rowholdout",
    "repeat": "seed43222_compact_repeat_dose2p64x_matched_rowholdout",
    "clean": "seed43222_cleanqwen_lengthmatched_dose2p64x_matched_rowholdout",
}
...
def count_jsonl(path: pathlib.Path) -> dict[str, Any]:
    ...
    return {
        ...
        "expected_updates_at_batch256": (rows + RECIPE["batch_size"] - 1) // RECIPE["batch_size"],
...
def stream_for(data_arm: str, meta: dict[str, Any]) -> tuple[pathlib.Path, str]:
    if data_arm in {"view", "repeat"}:
        key = "compact_view_dose2p64x" if data_arm == "view" else "compact_repeat_dose2p64x"
        sha_key = "compact_view_dose2p64x_100M.jsonl" if data_arm == "view" else "compact_repeat_dose2p64x_100M.jsonl"
        p = pathlib.Path(meta["files"]["training"][key])
        return (p if p.is_absolute() else ROOT / p), str(meta["sha256"][sha_key])
    if data_arm == "clean":
        return CLEAN_STREAM, EXPECTED_CLEAN_STREAM_SHA
    raise ValueError(data_arm)


def default_run_dir(data_arm: str) -> pathlib.Path:
    return RUNS_DIR / f"full_p2c_c2p_abs_{data_arm}_dose2p64x_matched_rowholdout_deberta100M_seed43222"
...
def preflight(data_arm: str, run_dir: pathlib.Path, count_words: bool) -> dict[str, Any]:
    ...
    if not (audit.get("all_exact_10M") and audit.get("row_length_sequence_identical_all_arms")):
        raise RuntimeError(f"unclean pool audit {audit}")
    if data_arm == "clean":
        if not CLEAN_MAT_PATH.exists():
            raise FileNotFoundError(CLEAN_MAT_PATH)
        mat = read_json(CLEAN_MAT_PATH)
        if pathlib.Path(mat.get("output_100m", "")).name != CLEAN_STREAM.name or mat.get("output_100m_sha256") != EXPECTED_CLEAN_STREAM_SHA:
            raise RuntimeError(f"clean materialization mismatch {mat}")
    else:
        mat = None
    stream, expected_stream_sha = stream_for(data_arm, meta)
    ...
    cmd = build_command(data_arm, run_dir, stream)
    return {
        "status": "SEED43222_TRAIN_PREFLIGHT_OK",
        "created_utc": now(),
        "data_arm": data_arm,
        "run_dir": rel(run_dir),
        "stream": rel(stream),
        ...
        "clean_materialization": rel(CLEAN_MAT_PATH) if mat is not None else None,
        "tokenizer_dir": rel(TOKENIZER),
        "tokenizer_json_sha256": tok_sha,
        "model_variant": mv,
        "recipe": RECIPE,
        "command": ["python3" if x == sys.executable else str(x) for x in cmd],
        "scientific_purpose": "Third-seed replication of the fixed-budget repetition-versus-variation computation trade-off: op0 direct recall should favor repeat, while deeper state-update Entity items should favor view, with clean anchoring V-C at identical row/update geometry.",
        "predicted_seed43222_entity_bands_late_80_90_100M": {
            "VminusR_op0": "negative and large, target around -9 pp; compatible band [-12,-6]",
            "VminusR_op3_or_op4": "positive and large, target around +7 to +9 pp; compatible band [+4,+12]",
            "VminusC_op4": "positive, target around +7.8 pp; compatible band [+5,+10]",
            "VminusC_op0": "near zero or mildly negative; compatible band [-3,+2]",
            "COMPS_VminusR_late": "near zero to slightly negative if COMPS is stored-property retrieval rather than state-update computation",
        },
...
def main() -> None:
    ...
    ap.add_argument("--data-arm", choices=["view", "repeat", "clean"], required=True)
    ...
    preflight_dir = FUNCTIONAL_RELATION_STUDIES_WS / "data" / "seed43222_train_preflight"
    ...
    if existing.get("ready"):
        print(json.dumps({"status": "SEED43222_ARM_ALREADY_FINISHED", "data_arm": args.data_arm, "run_dir": rel(run_dir), "metrics": existing}, indent=2, ensure_ascii=False), flush=True)
        return
    ...
    write_json(run_dir / "seed43222_train_preflight.json", info)
    ...
    env["TMPDIR"] = f"/tmp/q_relation_learning_step007_seed43222_{args.data_arm}_{int(time.time())}"
    ...
    start = now()
    start_rec = {"status": "SEED43222_ARM_TRAIN_STARTING", "data_arm": args.data_arm, "gpu": args.gpu, "run_dir": rel(run_dir), "created_utc": start, "scientific_purpose": info["scientific_purpose"], "no_evaluation_superglue_aoa_upload_or_leaderboard": True}
    print(json.dumps(start_rec, indent=2, ensure_ascii=False), flush=True)
    with stdout_path.open("w", encoding="utf-8") as out, stderr_path.open("w", encoding="utf-8") as err:
        out.write(json.dumps({"event": "seed43222_train_start", "utc": start, "data_arm": args.data_arm, "gpu": args.gpu, "recipe": RECIPE}, ensure_ascii=False) + "\n")
        out.flush()
        proc = subprocess.run(build_command(args.data_arm, run_dir, pathlib.Path(info["stream"]) if pathlib.Path(info["stream"]).is_absolute() else ROOT / info["stream"]), cwd=str(ROOT), env=env, stdout=out, stderr=err, text=True)
    result = {
        "status": "SEED43222_ARM_TRAIN_FINISHED" if proc.returncode == 0 else "SEED43222_ARM_TRAIN_FAILED",
    ...
    write_json(run_dir / "seed43222_train_launcher_result.json", result)
