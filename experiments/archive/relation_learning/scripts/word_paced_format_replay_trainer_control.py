...
_SCRIPT = _public_path('experiments/archive/relation_learning/scripts/word_paced_format_replay_trainer_control.py')
_ROOT = _public_path('.')
...
def load_fixed_batch(path: str, grouper: WordGrouper, tokenizer, rows: int, max_length: int = 256,
    ...
    toks = []
    for r in objs:
        ids, wg = grouper.tokenize(r["text"], max_length=max_length, add_special_tokens=add_special_tokens)
...
def train(args: argparse.Namespace) -> None:
    ...
    for r in rows:
        ids, wg = grouper.tokenize(r["text"], max_length=args.max_length, add_special_tokens=bool(args.add_special_tokens))
        tokenized.append({"ids": ids, "wg": wg, "words": int(r["words"])})

    leash_batch = load_fixed_batch(args.leash_coherent_stream, grouper, tokenizer, args.neutral_rows,
                                   max_length=args.max_length, add_special_tokens=bool(args.add_special_tokens)) if args.leash_coherent_stream else None
    readout_batch = load_fixed_batch(args.readout_coherent_stream, grouper, tokenizer, args.readout_rows,
                                    max_length=args.max_length, add_special_tokens=bool(args.add_special_tokens)) if args.readout_coherent_stream else None
...
def main() -> None:
    ...
    ap.add_argument("--add-special-tokens", dest="add_special_tokens", action="store_true", default=True)
    ap.add_argument("--no-add-special-tokens", dest="add_special_tokens", action="store_false")
