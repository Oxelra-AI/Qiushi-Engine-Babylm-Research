#!/usr/bin/env python3
"""research: guarded paired launcher for the FW compact-view vs source-breadth
mechanism comparison on the compliant DeBERTa-v2 8x480 recipe with ONE shared
16k tokenizer.

Expensive-work meaning (recorded before any GPU use)
----------------------------------------------------
The two arms are exactly matched (research mechanical audit): identical 10M/100M
word totals, identical row word sequence, identical source composition and
pass-order, one shared compliant tokenizer (SHA e70d167f...), zero hash overlap
between breadth companions and compact-pair sources. They differ ONLY in the
318,851 FineWeb companion words:
  - compact_view: each FineWeb source is followed by its faithful compact rewrite
    (aligned re-expression of the SAME propositions -> consolidation / density).
  - source_breadth: the same companion word budget is spent on INDEPENDENT extra
    FineWeb source sentences (additional experience -> breadth).

This paired run decides: does source-aligned compact re-expression create more
reusable/generalizable knowledge than spending the identical word budget on extra
independent FineWeb experience, under one shared tokenizer and matched row geometry?
  - compact wins  -> strengthens the aligned-compression SOTA route; reinvestment
    of saved words in MORE alignment is the mechanism.
  - breadth wins  -> the data substrate should shift toward broader source coverage;
    compact alignment is not the carrier.
  - similar       -> alternate wording contribution is unresolved; keep the research
    literal source_repeat arm as the next discriminating attribution run.

This is NOT leader imitation: it isolates a single data-mechanism factor while
holding tokenizer lineage, segmentation, WWM geometry, architecture, optimizer,
schedule, seeds, and row order identical.

Launch policy
-------------
Both H100s must be substantially free before launch. The paired comparison runs
one arm per GPU (arm A on GPU0, arm B on GPU1) with identical everything except
the example stream and run_dir. Default is --dry-run (preflight only). Actual
launch requires --launch and per-arm --gpu selection, and is normally driven by a
guarded shell wrapper that waits for free GPUs. Do not launch while the SGCR eval
or any other workload occupies a GPU.
"""
...
OUT_DIR = WS / "data/fw_compact_vs_breadth_train"
...
# 100M streams built and audited at research (same pass-order, distinct SHAs).
ARMS = {
    "compact_view": {
    ...
    "source_breadth": {
        "stream": WS / "data/fw_source_breadth_arm/fw_preserved_source_breadth_100M.jsonl",
        "sha256": "ba81476659a312ccea62056c152490d1b72ad88995a5ab5b3c4f79c873f956d6",
        "label": "fw_preserved_source_breadth_shared16k",
        "run_dir": AI_RUN_ROOT / "fw_source_breadth_shared16k_seed43022",
    },
}

# Compliant fixed recipe (matches research legal 8x480, only tokenizer/data differ).
...
def train_command(name: str, gpu: int | None) -> tuple[list[str], pathlib.Path]:
    a = ARMS[name]
    run_dir = a["run_dir"]
    cmd = [
...
def main() -> None:
    ...
    ap.add_argument("--arm", choices=list(ARMS.keys()), default=None,
                    help="Which arm to run when launching. Omit for dry-run of both.")
    ap.add_argument("--gpu", type=int, default=None, help="CUDA device index for this arm.")
    ap.add_argument("--launch", action="store_true", help="Actually launch training.")
    ap.add_argument("--full-word-check", action="store_true",
                    help="Count all words in each 100M stream (slower).")
    args = ap.parse_args()
    ...
    # Save both command manifests regardless of launch.
    manifest = {"status": "PAIRED_COMMANDS", "arms": {}}
    for name in ARMS:
        cmd, run_dir = train_command(name, None)
        manifest["arms"][name] = {"run_dir": str(run_dir), "cmd": cmd}
    (OUT_DIR / "paired_commands.json").write_text(
        json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    if not args.launch:
        print("\n[DRY RUN] Paired preflight passed. Commands ready; NOT launched.", flush=True)
        print("Launch one arm per GPU only when both H100s are free.", flush=True)
        return
    ...
    cmd, run_dir = train_command(args.arm, args.gpu)
    run_dir.mkdir(parents=True, exist_ok=True)
    env = dict(os.environ)
    env["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    print(f"Launching arm={args.arm} on GPU{args.gpu}; run_dir={run_dir}", flush=True)
    t0 = time.time()
    ...
    (OUT_DIR / f"train_result_{args.arm}.json").write_text(
