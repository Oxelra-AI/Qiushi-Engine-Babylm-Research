#!/usr/bin/env python3
"""research paired launcher for FW compact-view vs interleaved whole-sentence
source-breadth comparison.

This is the preferred repaired comparator after research: the breadth arm uses
coherent independent FineWeb sentences and places them between the same common
source segments, reducing within-row source-position differences relative to the
compact source/rewrite alternation. It supersedes research's sliced breadth stream
and is preferred over the research row-block whole-sentence breadth stream when the
first expensive comparison should minimize layout artifacts.
"""
...
OUT_DIR = WS / "data/fw_compact_vs_interleaved_breadth_train"
...
ARMS = {
    ...
    "source_breadth_interleaved_wholesentence": {
        "stream": WS / "data/fw_source_breadth_interleaved_wholesentence_arm/fw_preserved_source_breadth_interleaved_wholesentence_100M.jsonl",
        "sha256": "1758508e3bfae8bda8d4d66409ebd695499552a495e93d8327be8aa0d9182c44",
        "label": "fw_preserved_source_breadth_interleaved_wholesentence_shared16k",
        "run_dir": AI_RUN_ROOT / "fw_source_breadth_interleaved_wholesentence_shared16k_seed43022",
    },
}
...
def count_words(path: pathlib.Path) -> tuple[int, int]:
    total_words = 0
    rows = 0
...
def preflight(full_word_check: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "recipe": RECIPE,
        "seed": SEED,
        "preferred_comparator": "source_breadth_interleaved_wholesentence",
        "supersedes": "research source_breadth sliced stream; row-block whole-sentence breadth remains an auxiliary comparator, not the preferred first paired run.",
    }
...
def main() -> None:
    ...
    pf_path = OUT_DIR / "interleaved_paired_preflight.json"
    ...
    manifest = {"status": "INTERLEAVED_PAIRED_COMMANDS", "arms": {}}
    for name in ARMS:
        cmd, run_dir = train_command(name)
        manifest["arms"][name] = {"run_dir": str(run_dir), "cmd": cmd}
    (OUT_DIR / "interleaved_paired_commands.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    if not args.launch:
        print("\n[DRY RUN] Preferred interleaved compact-vs-breadth preflight passed. Commands ready; NOT launched.", flush=True)
        return
    ...
    print(f"Launching preferred research FW comparison arm={args.arm} on GPU{args.gpu}; run_dir={run_dir}", flush=True)
    t0 = time.time()
    ...
    (OUT_DIR / f"interleaved_train_result_{args.arm}.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
