#!/usr/bin/env python3
"""research: GlobalPIQA all-option readout for shared-tokenizer role-switch screen.

This is the hard-rank/margin readout for the legal from-scratch 80M screen that
uses a single shared tokenizer trained on the common 9,977,920-word intersection.
It compares role_switch_80M against role_fixed_80M and the matched fixed-256
legal40k 80M anchor. It performs no training.
"""
...
DEFAULT_OUT_ROOT = A01_WS / "data" / "globalpiqa_margin_reader"
...
def delta_summary(base: dict[str, Any], target: dict[str, Any]) -> dict[str, Any]:
    ...
    for mode in sorted(set(base.get("modes", {})) & set(target.get("modes", {}))):
        ...
        # Rank distribution changes are useful for hard52: rank 4 should fall, rank 1 should rise.
        if isinstance(b.get("always_wrong_subset"), dict) and isinstance(t.get("always_wrong_subset"), dict):
            br = b["always_wrong_subset"].get("rank_counts") or b["always_wrong_subset"].get("counts") or {}
            tr = t["always_wrong_subset"].get("rank_counts") or t["always_wrong_subset"].get("counts") or {}
...
def main() -> None:
    ...
    if "anchor_fixed256_80M" in summaries:
        for target in summaries:
            if target != "anchor_fixed256_80M":
                combined["deltas_vs_anchor_fixed256_80M"][target] = delta_summary(summaries["anchor_fixed256_80M"], summaries[target])
    if "role_switch_80M" in summaries and "role_fixed_80M" in summaries:
        combined["role_switch_minus_role_fixed"] = delta_summary(summaries["role_fixed_80M"], summaries["role_switch_80M"])

    combined_json = out_root / "globalpiqa_margin_summary.json"
