#!/usr/bin/env python3
"""research: repaired CPU-only seed-variance analysis for compact_view_reinvest.
...
OUT_DIR = WORKSPACE / "data/seed_variance_fast_and_dynamics_repaired"
NOTE = WORKSPACE / "notes/seed_variance_fast_and_dynamics_repaired.md"
...
def compare_recipe_and_order() -> dict[str, Any]:
    ...
    def normalized_recipe(cmd: dict[str, Any]) -> dict[str, Any]:
        # seed43022 stores all recipe fields under "recipe"; seed43122 stores
        # non-seed recipe fields under "fixed_recipe" and the seed controls at
        # top level. Normalize before comparison so storage-layout differences are
        # not read as scientific differences.
        rec: dict[str, Any] = {}
        if isinstance(cmd.get("recipe"), dict):
            rec.update(cmd["recipe"])
        if isinstance(cmd.get("fixed_recipe"), dict):
            rec.update(cmd["fixed_recipe"])
        for k in ["seed", "extra_init_seed", "train_rng_seed"]:
            if k in cmd:
                rec[k] = cmd[k]
        return rec

    recipe430 = normalized_recipe(cmd430)
    recipe431 = normalized_recipe(cmd431)
    ignored_rng_fields = {"extra_init_seed", "train_rng_seed"}
    recipe_diffs = {}
    for k in sorted(set(recipe430) | set(recipe431)):
        if k in ignored_rng_fields:
            continue
    ...
    return {
        ...
        "different_seeds": {
            "seed430_seed": recipe430.get("seed"),
            "seed431_seed": recipe431.get("seed"),
        ...
        "recipe_diffs_except_init_and_training_rng": recipe_diffs,
...
def main() -> None:
    ...
    analyses["Entity"] = analyze_entity("Entity", FAST_DATA["Entity"], filtered_nothing=True)
    ...
    summary = {
        "status": "SEED_VARIANCE_FAST_AND_DYNAMICS_REPAIRED",
        "created_utc": now(),
        "purpose": "Repaired CPU-only quantification of known seed43022 vs seed43122 endpoint variation while full seed43122 official-compatible evaluation remains managed asynchronously; fixes recipe seed-field normalization, fast Entity target filtering, CSV labels, and training-loss interpretation.",
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for col, a in analyses.items():
            ...
            for rec in a.get("worst_subtasks_for_seed431", [])[:10]:
                w.writerow({k: (col if k == "column" else rec.get(k)) for k in w.fieldnames})

    note_lines = []
    note_lines.append("# research — repaired seed variance before full seed43122 delivery\n")
    ...
    note_lines.append(f"- Recipe differences other than initialization/training RNG seeds: `{recipe['recipe_diffs_except_init_and_training_rng']}`.")
    note_lines.append(f"- Order manifest differences: `{recipe['order_manifest_diffs']}`.")
    note_lines.append(f"- Shared data-order seed: seed43022/seed43122={recipe['different_seeds']['seed430_seed']}/{recipe['different_seeds']['seed431_seed']}; changed initialization/training RNG seeds: seed43022={recipe['different_seeds']['seed430_extra_init_seed']}/{recipe['different_seeds']['seed430_train_rng_seed']}; seed43122={recipe['different_seeds']['seed431_extra_init_seed']}/{recipe['different_seeds']['seed431_train_rng_seed']}.\n")
    ...
    note_lines.append(f"- Final-step training loss: seed43022 {training['seed430']['loss_last']:.6f}, seed43122 {training['seed431']['loss_last']:.6f}, final-step delta {training['seed431']['loss_last'] - training['seed430']['loss_last']:+.6f}; last-100 matched-step mean delta {training['loss_delta_431_minus_430']['mean_last100']:+.6f}.")
    ...
    note_lines.append("- The late training loss is essentially tied, and mean matched-step loss is slightly lower for seed43122; the weaker fast endpoint is therefore not explained by a simple aggregate MLM-loss disadvantage. Loss still cannot predict the full official vector.\n")
    ...
    print(json.dumps({
        ...
        "recipe_diffs_except_init_and_training_rng": recipe["recipe_diffs_except_init_and_training_rng"],
