#!/usr/bin/env python3
"""research: legal 40k byte-BPE tokenizers for in-place packed replacement corpora.

The in-place corpora are the strict candidate data object for the from-scratch
Route B screen. This script trains each tokenizer only on its own revised 10M
corpus and audits treatment/control segmentation. It performs no model training.
"""
...
OUT = ROOT / 'experiments/archive/representation_and_objectives/data/inplace_replacement_tokenizers'
...
def train_tokenizer(label: str, corpus: pathlib.Path) -> pathlib.Path:
    out_dir = OUT / 'tokenizers' / f'{label}_inplace_legal_byte_bpe_40k'
    ...
    meta = {
        'status': 'trained', 'label': label, 'vocab_size_requested': VOCAB_SIZE,
        'vocab_size_actual': tok.get_vocab_size(), 'training_pool': str(corpus),
        'training_pool_sha256': sha256_file(corpus), 'tokenizer_json_sha256': sha256_file(tok_file),
...
def token_surface(tok, corpus: pathlib.Path) -> dict[str, Any]:
    ...
    for row_i, obj in enumerate(iter_rows(corpus)):
        ...
        if src.startswith('step146_'):
            packet_rows += 1; packet_words += words; packet_tokens += nt; repl_positions.append(row_i); packet_lengths.append(nt)
    packet_stats = None
    if packet_lengths:
        s = sorted(packet_lengths)
        packet_stats = {'n': len(s), 'min': min(s), 'mean': sum(s)/len(s), 'max': max(s), 'p50': s[len(s)//2], 'p90': s[int(0.9*(len(s)-1))], 'over_seq256': sum(x > SEQ for x in s)}
    return {
        'rows': rows, 'words': total_words, 'tokens': total_tokens,
        'tokens_per_word': total_tokens / total_words, 'rows_over_seq256': over_seq,
        'packet_rows': packet_rows, 'packet_words': packet_words, 'packet_tokens': packet_tokens,
        'packet_tokens_per_word': packet_tokens / packet_words if packet_words else None,
        'packet_row_token_stats': packet_stats,
        'packet_row_index_first_last': [repl_positions[0], repl_positions[-1]] if repl_positions else None,
...
def vocab_overlap(tok_a, tok_b) -> dict[str, Any]:
    ...
    return {'a_size': len(va), 'b_size': len(vb), 'shared': len(va & vb), 'a_only': len(va - vb), 'b_only': len(vb - va), 'shared_frac_a': len(va & vb)/len(va), 'shared_frac_b': len(va & vb)/len(vb), 'sample_a_only': sorted(va - vb)[:30], 'sample_b_only': sorted(vb - va)[:30]}
...
def tokenizer_json_delta(path_a: pathlib.Path, path_b: pathlib.Path) -> dict[str, Any]:
    ...
    return {'normalizer_equal': a.get('normalizer') == b.get('normalizer'), 'pre_tokenizer_equal': a.get('pre_tokenizer') == b.get('pre_tokenizer'), 'post_processor_equal': a.get('post_processor') == b.get('post_processor'), 'decoder_equal': a.get('decoder') == b.get('decoder'), 'vocab_dict_equal': va == vb, 'vocab_set_equal': set(va) == set(vb), 'merges_equal': a['model'].get('merges', []) == b['model'].get('merges', []), 'id_mismatch_count_common_tokens': len(mismatches), 'id_mismatch_examples': mismatches[:30]}
...
def main() -> None:
    ...
    tok_paths: dict[str, pathlib.Path] = {}
    ...
    for tlabel, tok in tokenizers.items():
        ...
        for arm, corpus in ARMS.items():
            ...
            s = surfaces[tlabel][arm]
            print(json.dumps({'event': 'surface_done', 'tokenizer': tlabel, 'corpus': arm, 'tpw': s['tokens_per_word'], 'over256': s['rows_over_seq256'], 'packet_tpw': s['packet_tokens_per_word'], 'packet_pos': s['packet_row_index_first_last']}), flush=True)
    summary = {
        'status': 'INPLACE_REPLACEMENT_TOKENIZERS',
        'legal_status': 'each tokenizer trained only on its own revised in-place packed 10M corpus; no model training performed',
        ...
        'role_switch_vs_role_fixed_tokenizer_json_delta': tokenizer_json_delta(tok_paths['role_switch'], tok_paths['role_fixed']),
        'special_ids': {label: {k: getattr(tok, k + '_token_id') for k in ['unk','bos','eos','pad','mask']} for label, tok in tokenizers.items()},
        'target_tokenization': {label: target_tokenization(tok) for label, tok in tokenizers.items()},
        'code_hash': sha256_file(_public_path('experiments/archive/representation_and_objectives/scripts/train_inplace_replacement_tokenizers.py')),
    }
    out_path = OUT / 'inplace_replacement_tokenizer_summary.json'
    out_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    print(json.dumps({'event': 'INPLACE_TOKENIZER_DONE', 'summary': str(out_path), 'paths': {arm: str(path) for arm, path in tok_paths.items()}}, indent=2), flush=True)
