#!/usr/bin/env python3
"""Materialize a seq256-safe, quality-filtered cached-FineWeb single-doc candidate.

Improvements over materialize_cached_fineweb_single_doc_candidate.py:
  1. Keep only single-document cached FineWeb rows (no cross-doc packing).
  2. Drop rows flagged by transparent quality checks (mojibake / html / non-Latin /
     low-alpha / symbol-index-like / bad substrings).
  3. Re-chunk the surviving FineWeb source into `--chunk-words` (default 112) rows so
     baseline16k seq256 truncation is near-zero and matches the official control block.

The treatment/control still share identical clean-Qwen pair rows, an identical official
tail, and an identical row-length sequence, so the only intended difference is
"broad factual FineWeb source" versus "official BabyLM text" in the replacement block.
This is a checked candidate, not a training launch.
"""
...
DEFAULT_OUT = pathlib.Path("experiments/archive/representation_and_objectives/training/data/cached_fineweb_seqsafe_candidate")
DEFAULT_NOTE = pathlib.Path("research/notes/representation_and_objectives/cached_fineweb_seqsafe_candidate.md")
...
def alpha_frac(text: str) -> float:
    ...
    if not nonspace:
        return 0.0
    alpha = sum(1 for ch in nonspace if unicodedata.category(ch).startswith("L"))
    return alpha / len(nonspace)


def is_quality(text: str) -> tuple[bool, list[str]]:
    flags = []
    low = text.lower()
    ...
    if words:
        ...
        if short > 0.32: flags.append("symbol_or_index_like")
        if digit_tok > 0.30: flags.append("many_digit_tokens")
...
def load_qwen_rows(path: pathlib.Path) -> list[Row]:
    ...
    for i, line in enumerate(path.open(encoding="utf-8")):
        ...
        if words != len(text.split()):
            raise RuntimeError(f"qwen word mismatch row {i}")
        rows.append(Row(text=text, words=words, source=str(o.get("source", "")), example_id=int(o.get("example_id", i))))
    return rows


def load_quality_single_doc_fineweb(path: pathlib.Path, chunk_words: int) -> tuple[list[Row], dict[str, Any]]:
    """Keep single-doc quality rows, then re-chunk their text to chunk_words rows."""
    kept_source_rows = []
    total_rows = 0; dropped_multi = 0; dropped_quality = 0
    doc_counter = collections.Counter(); flag_counter = collections.Counter(); examples = []
    for i, line in enumerate(path.open(encoding="utf-8")):
        if not line.strip():
            continue
        total_rows += 1
        ...
        kept_source_rows.append((doc, text, i))
        if len(examples) < 8:
            examples.append({"source_row": i, "doc_id": doc, "words": len(text.split()), "text_excerpt": text[:500] + ("…" if len(text) > 500 else "")})
    # Re-chunk kept source text into fixed word chunks; drop chunks < min_chunk to avoid tiny fragments.
    min_chunk = max(24, chunk_words // 2)
    out_rows = []
    eid = 8_100_000
    for doc, text, src_row in kept_source_rows:
        ws = text.split()
        for start in range(0, len(ws), chunk_words):
            part = ws[start:start + chunk_words]
            if len(part) < min_chunk:
                continue
            out_rows.append(Row(text=" ".join(part), words=len(part), source="fineweb_edu_random_quality_single_doc_seqsafe_cached_initial_model_studies", example_id=eid, meta={"doc_id": doc, "source_row": src_row, "chunk_start": start}))
            eid += 1
    return out_rows, {
        "input_rows": total_rows, "dropped_multi_doc_rows": dropped_multi, "dropped_quality_rows": dropped_quality,
        "kept_source_rows": len(kept_source_rows), "kept_source_words": sum(len(t.split()) for _, t, _ in kept_source_rows),
        "chunk_words": chunk_words, "min_chunk": min_chunk, "output_chunk_rows": len(out_rows), "output_chunk_words": sum(r.words for r in out_rows),
        "unique_docs": len(doc_counter), "rows_per_doc_top10": doc_counter.most_common(10), "quality_flag_counts": dict(flag_counter), "examples": examples,
    }
...
def chunk_to_lengths(state: StreamState, lengths: list[int], source: str, start_id: int):
    ...
    return rows, {"rows": len(rows), "words": sum(lengths), "component_sources_total": dict(comp_total), "multi_source_rows": multi, "multi_source_row_fraction": multi / len(rows) if rows else 0, "sample_meta": sample, "stream_consumed_words_after": state.consumed_words()}
...
def tail_rows(state: StreamState, needed: int, source: str, start_id: int):
    ...
    return rows, {"needed_words": needed, "rows": len(rows), "partial_rows": partial, "component_sources_total": dict(comp_total), "stream_consumed_words_after": state.consumed_words()}
    ...
    with path.open("w", encoding="utf-8") as f:
        for r in rows:
            if r.words != len(r.text.split()):
                raise RuntimeError(f"word mismatch before write {path}")
...
def main():
    ...
    ap.add_argument("--chunk-words", type=int, default=112)
    ...
    fineweb, fw_filter = load_quality_single_doc_fineweb(pathlib.Path(args.fineweb), args.chunk_words); fw_words = sum(r.words for r in fineweb)
    if qwen_words + fw_words >= args.total_words:
        raise RuntimeError("qwen+fineweb exceeds total")
    state = StreamState(filler)
    ctrl_block, ctrl_meta = chunk_to_lengths(state, [r.words for r in fineweb], "official_lengthmatched_to_seqsafe_fineweb", 8_600_000)
    tail_needed = args.total_words - qwen_words - fw_words
    tail, tail_meta = tail_rows(state, tail_needed, "official_identical_tail_after_seqsafe_fineweb_block", 9_100_000)
    ...
    required = {
        ...
        "fineweb_words_positive": fw_words > 0,
    ...
    paths = {
        "treatment_10M": out / "cleanqwen_seqsafe_fineweb_single_doc_10M.jsonl",
        "control_10M": out / "cleanqwen_official_lengthmatched_seqsafe_control_10M.jsonl",
        "treatment_100M": out / "cleanqwen_seqsafe_fineweb_single_doc_100M.jsonl",
        "control_100M": out / "cleanqwen_official_lengthmatched_seqsafe_control_100M.jsonl",
    ...
    if args.write_training:
        ...
        if e1 != e2:
            raise RuntimeError("training exposure mismatch")
        train_exp = e1
    ...
    verification = {
        "status": "CACHED_FINEWEB_SEQSAFE_CANDIDATE_VERIFIED",
        "total_words_per_pool": args.total_words, "passes": args.passes, "write_training": bool(args.write_training), "training_exposure_words": train_exp,
        "chunk_words": args.chunk_words,
        "rows": {"treatment": len(treat), "control": len(ctrl)},
        "source_word_counts_treatment": source_words(treat), "source_word_counts_control": source_words(ctrl),
        "qwen_pair_rows": len(qwen_pair), "qwen_pair_words": qwen_words,
        "fineweb_rows": len(fineweb), "fineweb_words": fw_words, "tail_filler_words": tail_needed,
        "word_fractions": {"clean_qwen_pairs": round(qwen_words / args.total_words, 6), "seqsafe_fineweb": round(fw_words / args.total_words, 6), "combined_non_tail_prefix": round((qwen_words + fw_words) / args.total_words, 6), "identical_official_tail": round(tail_needed / args.total_words, 6)},
        "row_stats": {"qwen_pair": stats([r.words for r in qwen_pair]), "fineweb": stats([r.words for r in fineweb]), "control_block": stats([r.words for r in ctrl_block]), "tail": stats([r.words for r in tail]), "pool": stats([r.words for r in treat])},
        "fineweb_filter": fw_filter,
        "control_block_from_official_filler": ctrl_meta, "identical_tail_from_official_filler": tail_meta,
        ...
        "required_checks": required, "all_required_checks_pass": True,
    }
    paths["verification"].write_text(json.dumps(verification, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    samples = {"fineweb_first_rows": fw_filter["examples"], "control_block_sample": ctrl_meta["sample_meta"][:8], "tail_first_source_counts": tail_meta["component_sources_total"]}
    paths["samples"].write_text(json.dumps(samples, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    meta = {"status": "CACHED_FINEWEB_SEQSAFE_CANDIDATE_MATERIALIZED", "created_unix": int(time.time()), "purpose": "seq256-safe, quality-filtered cached FineWeb single-doc factual-source candidate on top of clean-Qwen; not trained here", "inputs": {"qwen_pool": str(args.qwen_pool), "fineweb": str(args.fineweb)}, "outputs": {k: str(v) for k, v in paths.items()}, "verification": verification, "sha256": {k: sha256_file(v) for k, v in paths.items() if v.exists() and v.is_file() and k != "note"}, "elapsed_sec": round(time.time() - t0, 3)}
    paths["metadata"].write_text(json.dumps(meta, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    lines = ["# research seq256-safe cached FineWeb-Edu single-document candidate\n\n"]
    lines.append(f"Quality-filtered single-doc cached FineWeb text re-chunked to {args.chunk_words}-word rows so baseline16k seq256 truncation is near-zero and matches the official control block. Not trained evidence.\n\n")
    lines.append(f"- Clean-Qwen pair block: {len(qwen_pair):,} rows, {qwen_words:,} words ({qwen_words/args.total_words:.2%}).\n")
    lines.append(f"- Seq-safe FineWeb block: {len(fineweb):,} rows, {fw_words:,} words ({fw_words/args.total_words:.2%}); unique docs {fw_filter['unique_docs']:,}; dropped multi-doc {fw_filter['dropped_multi_doc_rows']:,}, dropped quality {fw_filter['dropped_quality_rows']:,}.\n")
    ...
    lines.append("- Treatment/control preserve identical clean-Qwen rows, identical tail, exact 10M words, identical row-length sequence.\n\n")
    ...
    print(json.dumps({"status": meta["status"], "out_dir": str(out), "chunk_words": args.chunk_words, "qwen_pair_words": qwen_words, "fineweb_words": fw_words, "fineweb_rows": len(fineweb), "tail_words": tail_needed, "unique_docs": fw_filter["unique_docs"], "dropped_quality_rows": fw_filter["dropped_quality_rows"], "rows": verification["rows"], "word_fractions": verification["word_fractions"], "all_required_checks_pass": True, "verification": str(paths["verification"]), "note": str(paths["note"])}, indent=2, ensure_ascii=False))
