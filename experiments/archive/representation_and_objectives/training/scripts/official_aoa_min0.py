#!/usr/bin/env python3
"""Generic official min_context=0 AoA evaluation for research+ local MLM ladders.

Uses the pristine current BabyLM strict evaluation checkout reconstructed in
research and computes AoA over the official strict-small checkpoint ladder
(chck_1M..chck_9M, chck_10M..chck_100M), loading 504 CDI words and 8,005
contexts per checkpoint (`min_context=0`). The model_root checkpoints supply
their own tokenizer, so research Strict-Small-tokenizer runs are evaluated under
...
def load_score_tokenizer(model_root: Path):
    for p in [model_root, model_root / "chck_100M", model_root / "chck_80M", model_root / "chck_10M", model_root / "chck_1M"]:
        if not p.exists():
            continue
        try:
...
def main() -> None:
    ...
    ap.add_argument("--model-root", type=Path, required=True, help="Local model root containing chck_* checkpoint directories")
    ...
    if args.dry_run:
        payload = {
            "status": "OFFICIAL_AOA_MIN0_DRYRUN",
            "created_utc": now_utc(),
            "tag": tag,
            "strict_repo": str(STRICT),
            "model_root": str(model_root),
            "model_root_record": file_record(model_root),
        ...
        out_json = out_dir / f"official_aoa_min0_{tag}_dryrun.json"
    ...
    if validation_errors:
        status = "OFFICIAL_AOA_MIN0_FAILED"
    else:
    ...
    payload = {
        ...
        "elapsed_sec": time.time() - t0,
        "interpretation": "Official-path AoA using min_context=0 (504 CDI words, 8005 contexts/checkpoint) and the local strict-small checkpoint ladder.",
    }
    out_json = out_dir / f"official_aoa_min0_{tag}.json"
    out_json.write_text(json.dumps(payload, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({
        "status": status,
        "aoa_raw_correlation": score,
