#!/usr/bin/env python3
"""Train/evaluate tiny matched entity-memory arms for research.

Arms: vanilla, shared_key, independent_key. The memory arms have identical
parameter counts. Shared_key uses one address projection for write and read;
independent_key uses separate projections. Evaluation includes write-key
permutation as a causal correspondence intervention.

Target encoding: each state word encodes as [space_prefix, discriminative_piece, ...]
under the legal 16k tokenizer. We use the first discriminative piece (index 1)
as the single MLM target at the mask position.
"""
...
def positions(row, tok, max_len):
    text = row["text"].replace("[MASK]", tok.mask_token)
    enc = tok(text, padding="max_length", truncation=True,
              max_length=max_len, return_offsets_mapping=True)
    offs = enc.pop("offset_mapping")

    # Entity slot positions: first mentions of the two entities
    slots = []
    for e in row["entities"][:2]:
        p, _ = find_token(offs, text, e)
        slots.append(p if p is not None else 0)
    if len(slots) == 1:
        slots.append(slots[0])

    # Initial state positions: first two state words
    state_hits = []
    for i, (a, b) in enumerate(offs):
        w = text[a:b].lower()
        if w in STATES and i not in state_hits:
            state_hits.append(i)
    if not state_hits:
        state_hits = [slots[0]]
    while len(state_hits) < 2:
        state_hits.append(state_hits[0])
    state_hits = state_hits[:2]

    # Ordered event verb and argument positions
    ev_pos = []
    cursor = 0
    for ev in row.get("events", []):
        vp, end = find_token(offs, text, ev["verb"], cursor)
        if vp is None:
            vp = slots[0]
        ep, end2 = find_token(offs, text, ev["entity"], end or cursor)
        if ep is None:
            ep = slots[0]
        ev_pos.append((vp, ep))
        cursor = end2 or cursor
    while len(ev_pos) < 3:
        ev_pos.append((-1, -1))
    ev_pos = ev_pos[:3]

    # Query entity position
    qterm = row["query_entity"]
    matches = list(re.finditer(r"\b" + re.escape(qterm) + r"\b", text, re.I))
    qa, qb = (matches[-1].span() if matches else (0, 1))
    qpos = slots[0]
    for i, (a, b) in enumerate(offs):
        if a < qb and b > qa:
            qpos = i
    ...
    # First discriminative BPE piece (index 1 skips shared space prefix 154)
    ...
    return {
        **enc,
        "slot_pos": slots, "state_pos": state_hits, "event_pos": ev_pos,
        "query_pos": qpos, "mask_pos": mpos,
...
def collate(xs):
    out = {}
    for k in ["input_ids", "attention_mask", "slot_pos", "state_pos",
              "event_pos", "query_pos", "mask_pos", "answer_id", "foil_id"]:
...
class EntityMemory(nn.Module):
    def __init__(self, d, arm):
        ...
        if arm == "shared_key":
            ...
            # Extra parameters to match independent_key count
    ...
    def forward(self, h, batch, permute_write=False, return_aux=False):
        ...
        sp = batch["slot_pos"]  # (B, 2)
        st = batch["state_pos"]  # (B, 2)

        slot_h = h[ar[:, None], sp]      # (B, 2, D)
        keys = self.slot_key(slot_h)     # (B, 2, D)
        slots = self.init_value(h[ar[:, None], st])  # (B, 2, D) initial values

        write_hist = []
        for t in range(batch["event_pos"].shape[1]):
            vp = batch["event_pos"][:, t, 0]
            ep = batch["event_pos"][:, t, 1]
            valid = vp.ge(0)
            vpc = vp.clamp_min(0)
            epc = ep.clamp_min(0)
            ...
            wlog = (q[:, None, :] * keys).sum(-1) * self.scale  # (B, 2)
            if permute_write:
                wlog = wlog.flip(1)
            wa = torch.softmax(wlog, dim=-1)  # (B, 2)
            ...
            # Write back to slots proportionally
        ...
        rlog = (rq[:, None, :] * keys).sum(-1) * self.scale
        ...
        aux = {
            "write_attention": torch.stack(write_hist, 1) if write_hist else torch.zeros(B, 0, 2, device=h.device),
            "read_attention": ra,
            "slots": slots,
        }
...
class TinyMLM(nn.Module):
    def __init__(self, vocab, arm, d=192, layers=4, heads=6, ff=768, max_len=64):
        ...
        self.memory = None if arm == "vanilla" else EntityMemory(d, arm)
        self.lm = nn.Sequential(
            nn.Linear(d, d), nn.GELU(), nn.LayerNorm(d),
            nn.Linear(d, vocab, bias=False),
        )
        self.lm[-1].weight = self.word.weight  # tie weights
...
def evaluate(model, loader, dev, permute=False):
    ...
    for b in loader:
        ...
        for i, m in enumerate(meta):
            group = m["split"]
            sums[group][0] += int(ok[i])
            sums[group][1] += 1
            sums[group][2] += float(margin[i])
            rows.append({
                "id": m["id"], "split": group, "kind": m["kind"],
                "family": m["family"], "pair_held": m["pair_held"],
                "correct": bool(ok[i]), "margin": float(margin[i]),
                "permuted": permute,
            })
        if aux:
            if aux["write_attention"].numel():
                wa_list.append(aux["write_attention"].cpu())
            ra_list.append(aux["read_attention"].cpu())

    summary = {k: {"correct": v[0], "n": v[1],
                    "accuracy": v[0] / v[1], "margin_mean": v[2] / v[1]}
               for k, v in sums.items()}
    transport = {}
    if ra_list:
        r = torch.cat(ra_list)
        transport["read_slot0_mean"] = float(r[:, 0].mean())
        transport["read_entropy_mean"] = float(
            (-(r * r.clamp_min(1e-9).log()).sum(-1)).mean())
    if wa_list:
        w = torch.cat(wa_list)
        transport["write_entropy_mean"] = float(
            (-(w * w.clamp_min(1e-9).log()).sum(-1)).mean())
    return summary, transport, rows


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--arm", choices=["vanilla", "shared_key", "independent_key"],
                    required=True)
    ...
    el = DataLoader(ev, batch_size=args.batch_size, shuffle=False,
                    collate_fn=collate, num_workers=0)
    dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = TinyMLM(len(tok), args.arm, args.d_model, args.layers,
                    max_len=args.max_len).to(dev)
    nparams = sum(p.numel() for p in model.parameters())
    print(json.dumps({"arm": args.arm, "parameters": nparams,
                      "device": str(dev), "train_records": len(train),
                      "eval_records": len(ev)}), flush=True)
    ...
    for ep in range(args.epochs):
        ...
        for b in tl:
            ...
            ar = torch.arange(logits.shape[0], device=dev)
            z = logits[ar, b["mask_pos"]]
            loss = nn.functional.cross_entropy(z, b["answer_id"])
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
            total += float(loss) * len(ar)
            n += len(ar)
    ...
    if args.arm == "vanilla":
        perm, perm_transport, perm_rows = {}, {}, []
    else:
        perm, perm_transport, perm_rows = evaluate(model, el, dev, True)

    torch.save({"model": model.state_dict(), "args": vars(args)}, out / "model.pt")
    with (out / "eval_rows.jsonl").open("w") as f:
        for r in rows + perm_rows:
            f.write(json.dumps(r) + "\n")

    result = {
        "status": "ENTITY_MEMORY_MINISCREEN",
        "arm": args.arm,
        "seed": args.seed,
        "device": str(dev),
        "dry_run": args.dry_run,
        "train_records": len(train),
        "train_words": sum(x["meta"]["word_count"] for x in train.items),
        "eval_records": len(ev),
        "epochs": args.epochs,
        "losses": losses,
        "parameters": nparams,
        ...
        "data_sha256": {
            "train": sha(Path(args.data_dir) / "train.jsonl"),
            "eval": sha(Path(args.data_dir) / "eval.jsonl"),
        },
        "baseline": base,
        "transport": transport,
        "write_permutation": perm,
        "write_permutation_transport": perm_transport,
