# seed variance fast and dynamics — seed variance before full seed43122 delivery

**Superseded by seed variance fast and dynamics repaired:** use `research/notes/representation_and_objectives/seed_variance_fast_and_dynamics_repaired.md` and `experiments/archive/representation_and_objectives/data/seed_variance_fast_and_dynamics_repaired/seed_variance_fast_and_dynamics.json` for seed-variance interpretation. seed variance fast and dynamics missed seed43122 top-level seed fields, used an unfiltered Entity-fast target alignment, omitted CSV column labels, and overstated aggregate training-loss separation.


Machine-readable JSON: `experiments/archive/representation_and_objectives/data/seed_variance_fast_and_dynamics/seed_variance_fast_and_dynamics.json`
Worst-subtask CSV: `experiments/archive/representation_and_objectives/data/seed_variance_fast_and_dynamics/seed_variance_worst_fast_subtasks.csv`
...
- Recipe differences other than init/RNG seeds: `{'seed': {'seed430': 43, 'seed431': None}}`.
- Order manifest differences: `{}`.
- Changed seeds: seed43022 extra_init/train_rng=43022/43023; seed43122 extra_init/train_rng=None/None.
...
## Training dynamics
- Final training loss: seed43022 2.565617, seed43122 2.641525, delta +0.000336 mean over the last 100 matched steps.
- Mean loss over all matched steps delta seed43122−seed43022: -0.003585.
- The training-loss disadvantage is broad and late, so the fast endpoint gap is not merely a reporting artifact; however, loss is not sufficient to predict the full official vector.
...
- Entity: agreement 0.6099, only430_correct=5, only431_correct=6, net 1; worst subtasks: regular_1_ops -0.49; regular_0_ops +0.00; regular_3_ops +0.00; regular_2_ops +0.25; regular_4_ops +0.26
